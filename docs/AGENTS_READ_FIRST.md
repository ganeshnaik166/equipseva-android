...
| **Backend regression suite + round 3824 applied** — a grants ratchet joins every `.rpc("…")` call site against the EXECUTE privileges each API role holds. Its first run found 27 founder-console RPCs the console could never execute (it runs as `authenticated`, they were `service_role`-only) and an `anon`-reachable SECURITY DEFINER write loop. Round 3824 fixed both and IS APPLIED to prod. | `supabase/regression/`, `supabase/migrations/20263901000000_round3824_*.sql`, `.github/workflows/backend.yml` | before you write a migration, run `node supabase/regression/check.mjs` — it fails on a new missing/ungranted RPC. Refresh `baseline/` ONLY in the commit that intentionally changes the schema. |
| **~120 audited findings, committed** — seven read-only scopes over auth/nav, the data layer.s DTO-to-SQL contracts, the repair and money journeys, the outbox/security surface, Compose, and the newest migrations plus edge functions; each entry has file:line, evidence, a minimal fix and the test that pins it. | `docs/audit-20260917/` on `claudedev-quality-20260912` | do not re-derive them; pick an unfixed id and implement it. Priority order is in `docs/HANDOFF_CLAUDEDEV_QUALITY_20260918.md`. |
