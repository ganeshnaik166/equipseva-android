...
## 2026-09-23 — claudedev-build-20260923: KYC prefill adopts the catalog parser (first user-visible consumer)

- Commit `6492070e` on `claudedev-build-20260923` (Claude), after `48196da0` recorded the scope. `KycViewModel.hydrate()` now reads `engineers.city` through `parseLegacyCity(raw, knownState = engineer.state)` and prefills only explicitly stated State/district; `onServiceCoordsChange()` canonicalises Geocoder labels through `canonicalState` / `canonicalDistrict(allowEmbedded = true)`. Only those two regions changed (outside every Codex-candidate hunk in the file). Location round-3 nits folded in (`stateInferred`, post-State slot rule, spaced pincode, "Distt").
- Verified: targeted `KycViewModelTest` + `core.data.location.*` + `i18n.*` — 82 tests / 0 failures; full bar `:app:testDebugUnitTest :app:lintDebug :app:assembleDebug` — BUILD SUCCESSFUL in 3m 4s: **2,971 unit tests / 345 suites / 0 failures / 0 errors / 0 skipped** (7 new on top of 2,964); lint **0 errors, 86 warnings, 2 hints**; `app-debug.apk` built (47.3 MB).
- Reviews: critic **9.6 accepted**, QA **9.7 accepted** for the declared scope; their follow-ups (unit-word rejection in prefill matching, a differential Geocoder test, no third-source prefill on a conflict) landed in `6ea06f1d`.
- Branch still not integrated to `main`; PR to be opened after CI. Handoff: [HANDOFF_CLAUDEDEV_BUILD_2026-09-23.md](HANDOFF_CLAUDEDEV_BUILD_2026-09-23.md).

## 2026-09-23 — claudedev-build-20260923: N01 registration-intent chooser prepared (unwired) and slice-3 follow-ups

- Commits `fa2b20fc` (scope recorded first), `3d438e0e` (Codex `d202cb38` cherry-picked verbatim: `PublicRegistrationIntent`, `RegionSelectionDraft`, tests), `85fbb27e` (`features/auth/RegistrationIntentContent.kt`, `registration_intent_*` strings, `RegistrationIntentContentUiTest` 5 tests; no route, ViewModel, navigation, role, membership or entitlement effect) and `6ea06f1d` (slice-3 review follow-ups: unit-word rejection in prefill district matching, differential Geocoder VM test, no third-source prefill on a text-vs-column conflict, strict profile fallback pinned).
- Verified on the tip: targeted 114 tests / 0 failures across the ten affected classes; full bar 3,003 tests / 348 suites / 0 failures / 0 errors, lint 0 errors / 86 warnings / 2 hints, `assembleDebug` built.
- Reviews: slice 3 (`6492070e`) critic **9.6** and QA **9.7**, both accepted for the declared scope; slice 4 + follow-ups: critic **9.7 / 9.7** and QA **9.8 / 9.8** (chooser / follow-ups), all accepted for the declared scope; the reviewers' nits (chooser subtitle and hospital copy, savedKey-vs-storageKey invariant over all purposes, spacer idiom, conflict guard only with a State column) landed in `89fa39a6`.
- Branch still not integrated to `main`; PR after CI (the android run for the intermediate commit `6492070e` was cancelled by the branch concurrency rule when newer commits were pushed; the tip run is the authoritative one). Handoff: [HANDOFF_CLAUDEDEV_BUILD_2026-09-23.md](HANDOFF_CLAUDEDEV_BUILD_2026-09-23.md).
