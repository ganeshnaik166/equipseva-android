...
| P1.3 | Safe owner/workspace queue, photos, caches and push | Architecture mapped; [P1d source gap](../helper-reviews/codex-20260926/p1d-remote-token-gap.md), [security critique](../helper-reviews/codex-20260926/p1d-contract-critic.md) and [P1d-1 read-only plan](../helper-reviews/codex-20260926/p1d1-schema-readonly-plan.md) (planning critic 9.6 only) | Partial local P1a–P1c protections; P1d-0 sender payload/error-classification passes 13/13 Node and both Deno versions, scoped helper and integration critic/QA 9.6 each. Tracked `device_tokens` DELETE revoke conflicts with client DELETE-before-upsert; live grants unknown | Sender-only [PR1886](https://github.com/ganeshnaik166/equipseva-android/pull/1886) merged main `7915f04a` after all seven checks; no Supabase deployment. Broad Android candidate and remote claim/release blocked | Actual DB schema, server-fenced claim/release, two-session races, old-client rollout, delayed producers/readers, process death, revocation, reconnect and provider/device privacy |
...
| P2.2 | Versioned State/UT and district contract | Planned | Partial: client foundation on `claudedev-build-20260923` (`23cc3ac5`, `22d343ac`, `44b1dcbe`) — catalog header reconciled and pinned, `CATALOG_VERSION`, `KNOWN_MISSING`, LGD State/UT codes, legacy alias tables, needs-confirmation parser for the real `engineers.city` shapes; names only, no district codes, no caller migrated; independent critic **9.7** and QA **9.7** accepted for this declared scope (full bar 2,964 / 0, lint 0 errors) per the [handoff](../HANDOFF_CLAUDEDEV_BUILD_2026-09-23.md) | Branch only; not on main | Dated LGD asset with district codes, server pair validation, legacy alias/rollback tests against the coded dataset, KYC caller adoption |
...
| P3.1 | Three-choice entry and returning-user workspace selection | Planned | Existing two-persona live root only; signup-footer nav PR1889 merged at `edae85ed` with scoped critic 9.7 / QA 9.6 and seven GitHub checks; Welcome accessibility PR1888 remains under integration; the N01 chooser composable is prepared and tested on `claudedev-build-20260923` (`85fbb27e`, unwired, grants nothing) | New org entry absent; full three-choice flow absent; chooser not routed | Native auth/provider recovery; no paid/global-role shortcut; P2 tenant contracts before wiring |
...
| P7.1 | Remaining account/chat/AMC/admin redesign | Full page carry-forward map | Partial reusable components/legacy screens; corrected Welcome source `9abd0ffc` has 5/0 focused and 2,912/0 full local tests, design lint/lint/debug/unsigned R8 green, reviewed normal/compact probes, critic 9.7 and QA 9.6 for that screen only | [Welcome PR1888](https://github.com/ganeshnaik166/equipseva-android/pull/1888) final-head CI/main pending; wider UI partial | Physical TalkBack/device, accessibility and affected runtime flows (English only); three-purpose entry and Space Grotesk/Inter remain open |
| P7.2 | Complete owned-source review and dependency disposition | Review plan + bounded existing audits | Incomplete overall; corrected Sharp 0.35.4 lockfile `13bef61e` retains 16 Linux `libc` selectors, fresh local and Node 20/Linux CI green, scoped critic 9.6 and QA 9.7 | PR1885 merged main `a645a593`; GitHub alert #17 shows Fixed; other fixes partial | Hosted runtime/deployment and final-hash review ledger; other open HIGH/security/payment findings remain |
