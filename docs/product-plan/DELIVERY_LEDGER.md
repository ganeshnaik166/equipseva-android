...
| P7.1 | Remaining account/chat/AMC/admin redesign | Full page carry-forward map | Partial reusable components/legacy screens; isolated SignIn recovery target `ffbc930f` has test-first RED 3/2, GREEN 3/0 and full local 2,910/0 with lint/debug/unsigned R8 green; scoped critic 9.7/QA 9.6 pass only that target | SignIn candidate branch only; broader redesign remains partial | Hosted CI, physical TalkBack/device, hosted reset and affected runtime flows (English only) |
