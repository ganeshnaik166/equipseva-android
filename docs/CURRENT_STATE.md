...
The P1b candidate checkpoint is pushed; this docs-only branch proposes its safe continuity records for main while app code stays blocked. Next implementation slice is a separately owned P1c token-capture fix: capture before the first draft disk suspension, bind it to the first-operation ticket, preserve the old token regression and test A→B/same-account replacement. Final SDK logout, other cleanup resources and S3 recovery remain separate. No new DAO schema/API is needed for P1b.
