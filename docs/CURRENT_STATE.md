...
P1b checkpoint was pushed at `11b43ee97a15afde2cf62d796f8905cbe33164dc`, with matching remote SHA and clean tree; safe continuity docs are being published separately to main. **Active next slice P1c: departing device-token capture**. Coordinator owns only `core/auth/SignOutCleanup.kt`, `core/data/repair/RequestServiceDraftStore.kt` and `core/push/DeviceTokenRegistrar.kt` production changes, Gradle, Git and handoff. A separate test agent owns the new focused token-capture regression class and limited updates to `SignOutCleanupLocalBoundaryRegressionTest.kt`, `SignOutCleanupOwnershipTest.kt`, `SignOutDraftFenceTest.kt`, `DeviceTokenRevocationBoundaryRegressionTest.kt` and `RequestServiceDraftStoreTest.kt`; no test deletions or weakened B-survival assertions. Capture the token before the first draft disk suspension, bind it to the first-operation exact ticket, reject a stale A during the suspending token DAO read, and fence only A's draft lease synchronously without fencing B. First prove failing tests against the unchanged P1b production; then implement and run focused/full checks. Final SDK logout, other global cleanup resources and S3 recovery remain separate. Do not touch the parallel English-only/region branch, Room schema, Outbox, AMC, backend or UI.
