...
3. Frozen test-first implementation plan (§4).
4. Two **test fixtures** (no tests yet):
   - `app/src/test/kotlin/com/equipseva/app/testing/RecordingUserPrefs.kt` — MockK-backed
     ...
     `beforeActiveRoleWrite` suspension hook.
...
- CI has **not** run: every workflow's `push.branches` filter is
  `[main, "ops/**", "codex/**", "claudedev-help"]`; `claudedev-auth-audit` is excluded. Open a
  PR or add the branch to the filter (workflow files were deliberately not edited here).
...
## 4. Frozen test-first implementation plan
...
| A4 (F9) | `features/auth/RoleSelectViewModelTest`: `late updateRole success after account switch does not write the role` — target expectation `activeRoleWrites == []`; characterisation pins the stale write. | Re-read `sessionState.first()` after `updateRole` and compare `userId` before `setActiveRole`; same guard in `SignUpViewModel`, `ProfileViewModel`. | Green; `FakeAuthRepository.setSession` switch mid-flight covered for all three writers. |
...
| A9 (F7) | `SessionViewModelStateMachineTest`: state sequence on fresh sign-in of an onboarded account is `[Loading, Ready]` (target); characterisation pins `NeedsOnboarding` in between via `RecordingUserPrefs.beforeActiveRoleWrite`. | Set `profileOnboardingV2Complete` **before** `setActiveRole`, or hold `bootstrapping=true` until both are written. | Sequence test green. |
...
| A12 (F12) | `core/auth/SignOutOrderingTest`: `signOut` failure keeps a recoverable state (target). | `ProfileViewModel.onSignOut`: revoke token first, then `signOut`, then wipe; surface failure. | Green. |
...
| `app/src/test/kotlin/com/equipseva/app/testing/RecordingUserPrefs.kt` | UserPrefs test double (draft, uncompiled) |
| `app/src/test/kotlin/com/equipseva/app/testing/AuthAuditFixtures.kt` | Profile fixtures (draft, uncompiled) |
...
| --- | --- | --- | --- |
| m1 | (filled in by the commit that adds this file — see `git log -1`) | audit doc + two fixtures | **none** — not compiled, not run, no CI (branch excluded from workflow filters) |
