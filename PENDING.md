...
**Follow-up test backfill (2026-05-20):** the v1-test gap from PR #255 is closed. `:app:testDebugUnitTest` grew from 63 → 844 tests across **PR #925** (wave-1, 63 → 316) and **PR #926** (wave-2, 316 → 844). Wave-1 covered the load-bearing wire contracts + DTO mappers + validators + KYC stepper. Wave-2 used 11 parallel sub-agents across 4 rounds + 1 final sweep to widen coverage into every ViewModel's pure derivation (UiState gates, partitioners, search filters, fold resolvers), every Supabase repo's pure helpers (insert-DTO builders, sender-gate decision, retry classifier), the founder dashboard / queues, design-system pills + stepper, RepairJobCard equipment-icon classifier, cost-revision banner + delta, AuthError full classifier, Postgrest error classifier, chat realtime typing TTL + day-grouping helpers, storage object-key validator, photo-upload outbox routing, role-tile picker copy, IndiaLocations catalog, DashboardCommon compact ₹ formatter. ~40 small main-source extractions across the two PRs — all behavior-preserving — pulled private helpers into top-level `internal fun` so the math is testable without Compose / Supabase standup.
