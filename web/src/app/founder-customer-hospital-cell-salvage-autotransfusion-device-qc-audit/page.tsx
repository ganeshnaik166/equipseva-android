...
type DigestRow = {
  ...
  air_detector_fail: number;
  avg_deviation_pct: number | null;
  worst_deviation_pct: number | null;
...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3443_qc_verdict_rollup'),
    supabase.rpc('founder_r3443_device_model_scorecard'),
    supabase.rpc('founder_r3443_parameter_verdict_matrix'),
    supabase.rpc('founder_r3443_monthly_calibration_trend'),
    supabase.rpc('founder_r3443_capa_status_board'),
    supabase.rpc('founder_r3443_root_cause_pareto'),
    supabase.rpc('founder_r3443_accuracy_impact_digest'),
    supabase.rpc('founder_r3443_high_risk_queue'),
  ]);
  ...
  const modelCols: Column<ModelRow>[] = [
    ...
    { key: 'air_detector_fail', header: 'Air Detector Fail' },
  ...
  const digestCols: Column<DigestRow>[] = [
    ...
    { key: 'failed_checks', header: 'Failed' },
  ];
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Cell-Salvage / Autotransfusion Device QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital intra-op cell-salvage / autotransfusion (Cell Saver) machine QA log — parameter
        (centrifuge RPM, wash volume ml, processed hematocrit %, pump flow, air detector, waste valve)
        &times; device model &times; reference &amp; measured value &times; deviation % &times;
        air-detector integrity &times; calibration date &amp; CAPA closure. Founder-gated view: QC
        verdicts, device-model scorecards, accuracy-impact digest, root-cause pareto and out-of-tolerance
        / air-detector-fail queue across NABH &amp; CDSCO surfaces.
      </p>
