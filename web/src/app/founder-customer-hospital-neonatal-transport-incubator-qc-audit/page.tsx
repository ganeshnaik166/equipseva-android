...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3539_qc_verdict_rollup'),
    supabase.rpc('founder_r3539_device_model_scorecard'),
    supabase.rpc('founder_r3539_parameter_verdict_matrix'),
    supabase.rpc('founder_r3539_monthly_accuracy_trend'),
    supabase.rpc('founder_r3539_capa_status_board'),
    supabase.rpc('founder_r3539_root_cause_pareto'),
    supabase.rpc('founder_r3539_accuracy_impact_digest'),
    supabase.rpc('founder_r3539_high_risk_queue'),
  ]);
  ...
  const trendCols: Column<TrendRow>[] = [
    ...
    { key: 'failed', header: 'Failed' },
    { key: 'alarm_fail', header: 'Alarm Fail' },
    { key: 'avg_abs_deviation_pct', header: 'Avg Abs Deviation %' },
  ];
  ...
  const digestCols: Column<DigestRow>[] = [
    { key: 'deviation_band', header: 'Deviation Band' },
    { key: 'checks', header: 'Checks' },
    { key: 'out_of_tolerance', header: 'Out of Tol' },
    { key: 'alarm_fail', header: 'Alarm Fail' },
    { key: 'avg_abs_deviation_pct', header: 'Avg Abs Deviation %' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Neonatal Transport Incubator QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Neonatal transport incubator QA log — device model &times; transport context (inter-hospital,
        intra-hospital, ambulance &amp; air transport) &times; parameter (air &amp; skin temp, battery
        runtime, O2 concentration, alarm response, vibration damping) &times; reference vs measured
        &times; deviation % &times; alarm test &times; battery health &times; calibration currency
        &amp; CAPA closure. Founder-gated view: QC verdicts, device-model scorecards, root-cause pareto,
        and accuracy-impact digest across NABH &amp; CDSCO surfaces.
      </p>
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        <DataTable
          ...
          rowKey={(r, i) => String(r.deviation_band ?? i)}
        />
      </section>
