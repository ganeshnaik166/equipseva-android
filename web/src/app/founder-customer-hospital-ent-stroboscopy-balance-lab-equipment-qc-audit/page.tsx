...
type RiskRow = {
  ...
  check_date: string;
  qc_verdict: string;
  strobe_sync_accuracy_ok: string | null;
  camera_image_quality: string | null;
...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3314_qc_verdict_rollup'),
    supabase.rpc('founder_r3314_hospital_scorecard'),
    supabase.rpc('founder_r3314_device_type_department_matrix'),
    supabase.rpc('founder_r3314_daily_qc_trend'),
    supabase.rpc('founder_r3314_capa_status_board'),
    supabase.rpc('founder_r3314_root_cause_pareto'),
    supabase.rpc('founder_r3314_regulatory_impact_digest'),
    supabase.rpc('founder_r3314_high_risk_queue'),
  ]);
  ...
  const hospCols: Column<HospRow>[] = [
    ...
    { key: 'strobe_sync_fail', header: 'Strobe-Sync Fail' },
    { key: 'camera_fail', header: 'Camera Fail' },
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'check_date', header: 'Date' },
    { key: 'qc_verdict', header: 'Verdict' },
    { key: 'strobe_sync_accuracy_ok', header: 'Strobe-Sync' },
    { key: 'camera_image_quality', header: 'Camera' },
    { key: 'leak_test_scope', header: 'Leak Test' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Customer Hospital ENT Voice &amp; Balance-Lab Equipment QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        ENT QA log — device type &times; light-source output &times; strobe-sync accuracy &times;
        camera image quality &times; scope fiber integrity &amp; leak test &times; VNG calibration
        &times; posturography force-plate calibration &times; infection control &amp; calibration
