...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3511_qc_verdict_rollup'),
    supabase.rpc('founder_r3511_device_model_scorecard'),
    supabase.rpc('founder_r3511_parameter_verdict_matrix'),
    supabase.rpc('founder_r3511_monthly_accuracy_trend'),
    supabase.rpc('founder_r3511_capa_status_board'),
    supabase.rpc('founder_r3511_root_cause_pareto'),
    supabase.rpc('founder_r3511_accuracy_impact_digest'),
    supabase.rpc('founder_r3511_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Orthopedic Traction Unit (Cervical/Lumbar) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital orthopedic traction unit QA log — device model &times; parameter (set force,
        measured force, hold time, ramp rate, timer accuracy, e-stop response) &times; reference vs
        measured value &times; deviation % &times; within-tolerance &times; calibration currency
