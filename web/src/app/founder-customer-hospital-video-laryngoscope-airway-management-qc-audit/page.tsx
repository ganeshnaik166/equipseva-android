...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3459_qc_verdict_rollup'),
    supabase.rpc('founder_r3459_device_model_scorecard'),
    supabase.rpc('founder_r3459_parameter_verdict_matrix'),
    supabase.rpc('founder_r3459_monthly_calibration_trend'),
    supabase.rpc('founder_r3459_capa_status_board'),
    supabase.rpc('founder_r3459_root_cause_pareto'),
    supabase.rpc('founder_r3459_accuracy_impact_digest'),
    supabase.rpc('founder_r3459_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Video Laryngoscope (Airway Management) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital video laryngoscope airway-management QA log — parameter (image resolution, light
        output lux, battery runtime, anti-fog, display latency &amp; blade integrity) &times; device
        model (C-MAC, GlideScope, McGrath MAC, King Vision, Airtraq) &times; reference vs measured
        &times; deviation % &times; tolerance band &times; calibration date &amp; CAPA closure.
