...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3535_qc_verdict_rollup'),
    supabase.rpc('founder_r3535_device_model_scorecard'),
    supabase.rpc('founder_r3535_parameter_verdict_matrix'),
    supabase.rpc('founder_r3535_monthly_calibration_trend'),
    supabase.rpc('founder_r3535_capa_status_board'),
    supabase.rpc('founder_r3535_root_cause_pareto'),
    supabase.rpc('founder_r3535_accuracy_impact_digest'),
    supabase.rpc('founder_r3535_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Nerve-Integrity Monitor (NIM / EMG) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Intra-operative nerve integrity monitor (NIM / EMG) QA log — device model &times; parameter
        (stim current mA, EMG detection threshold &micro;V, event latency ms, electrode impedance
        k&Omega;, artifact rejection &amp; audio alert dB) &times; reference vs measured &times;
        deviation % &times; tolerance &times; calibration date &times; verdict &amp; CAPA closure.
