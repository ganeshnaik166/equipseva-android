...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3451_qc_verdict_rollup'),
    supabase.rpc('founder_r3451_device_model_scorecard'),
    supabase.rpc('founder_r3451_laser_verdict_matrix'),
    supabase.rpc('founder_r3451_monthly_calibration_trend'),
    supabase.rpc('founder_r3451_capa_status_board'),
    supabase.rpc('founder_r3451_root_cause_pareto'),
    supabase.rpc('founder_r3451_accuracy_impact_digest'),
    supabase.rpc('founder_r3451_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Flow Cytometer Fluorescence-Calibration QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital flow cytometer fluorescence &amp; scatter calibration QA log — measured parameter
        (FSC/SSC %CV, FL1 &amp; FL2 median fluorescence intensity, laser delay, bead recovery)
        &times; excitation laser (blue 488, red 640, violet 405, UV 355) &times; reference vs
        measured value &times; deviation % &times; within-tolerance &times; QC verdict &amp; CAPA
        closure. Founder-gated view: QC verdicts, device-model scorecards, root-cause pareto, and
        accuracy-impact digest across NABL &amp; CDSCO surfaces.
      </p>
      ...
      <section style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.2rem', marginBottom: '0.5rem' }}>3. Laser &times; verdict matrix</h2>
        <DataTable
          rows={matrixRows}
          columns={matrixCols}
          emptyMessage="No checks by laser."
          rowKey={(r, i) => `${r.laser}-${r.qc_verdict}-${i}`}
        />
      </section>
