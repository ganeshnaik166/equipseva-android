...
type RiskRow = {
  ...
  tolerance_limit_pct: number | null;
  cv_pct: number | null;
  notes: string | null;
};
...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3475_qc_verdict_rollup'),
    supabase.rpc('founder_r3475_device_model_scorecard'),
    supabase.rpc('founder_r3475_parameter_verdict_matrix'),
    supabase.rpc('founder_r3475_monthly_trend'),
    supabase.rpc('founder_r3475_capa_status_board'),
    supabase.rpc('founder_r3475_root_cause_pareto'),
    supabase.rpc('founder_r3475_accuracy_impact_digest'),
    supabase.rpc('founder_r3475_high_risk_queue'),
  ]);
  ...
  const modelCols: Column<ModelRow>[] = [
    ...
    { key: 'avg_abs_deviation_pct', header: 'Avg |Deviation| %' },
  ...
  const impactCols: Column<ImpactRow>[] = [
    { key: 'severity_band', header: 'Deviation Band' },
    { key: 'checks', header: 'Checks' },
    { key: 'out_of_tolerance', header: 'Out of Tolerance' },
    { key: 'failed', header: 'Failed' },
    { key: 'avg_abs_deviation_pct', header: 'Avg |Deviation| %' },
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'tolerance_limit_pct', header: 'Tolerance %' },
    { key: 'cv_pct', header: 'CV %' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Micropipette Gravimetric-Calibration QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital laboratory micropipette gravimetric calibration QA log — device model &times;
        parameter (accuracy %, precision CV %, 10 &micro;L / 100 &micro;L / 1000 &micro;L volume,
        leak test) &times; reference vs measured value &times; deviation % &times; tolerance limit
        &times; replicate CV &times; verdict &amp; CAPA closure. Founder-gated view: QC verdicts,
        device-model scorecards, parameter matrix, deviation-severity digest, root-cause pareto, and
        high-risk queue across NABL &amp; ISO 15189 surfaces.
      </p>
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        <DataTable
          rows={impactRows}
          columns={impactCols}
          emptyMessage="No deviation-band data."
          rowKey={(r, i) => String(r.severity_band ?? i)}
        />
      </section>
