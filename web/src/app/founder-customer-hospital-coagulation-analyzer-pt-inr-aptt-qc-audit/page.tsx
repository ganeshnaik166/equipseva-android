...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3450_qc_verdict_rollup'),
    supabase.rpc('founder_r3450_device_model_scorecard'),
    supabase.rpc('founder_r3450_parameter_verdict_matrix'),
    supabase.rpc('founder_r3450_monthly_accuracy_trend'),
    supabase.rpc('founder_r3450_capa_status_board'),
    supabase.rpc('founder_r3450_root_cause_pareto'),
    supabase.rpc('founder_r3450_accuracy_impact_digest'),
    supabase.rpc('founder_r3450_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Coagulation Analyzer (PT/INR/aPTT) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital coagulation analyzer QA log — parameter (PT sec, INR, aPTT sec, fibrinogen mg/dL,
        D-dimer, control level) &times; device model (Sysmex, Stago, Werfen) &times; reference vs
        measured value &times; deviation % &times; QC level (normal, abnormal-low, abnormal-high)
        &times; tolerance &times; calibration date &times; verdict &amp; CAPA closure. Founder-gated
        view: QC verdicts, device-model scorecards, root-cause pareto, and accuracy-impact digest
        across NABL &amp; CDSCO surfaces.
      </p>
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        <DataTable
          ...
          rowKey={(r, i) => String(r.qc_month ?? i)}
        />
      </section>
