...
type RiskRow = {
  ...
  check_date: string;
  qc_verdict: string;
  scope_image_quality: string | null;
  fluoroscopy_dose_rate_ok: string | null;
  radiation_safety_ok: string | null;
  scope_leak_test: string | null;
  notes: string | null;
};
...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3315_qc_verdict_rollup'),
    supabase.rpc('founder_r3315_hospital_scorecard'),
    supabase.rpc('founder_r3315_device_dept_matrix'),
    supabase.rpc('founder_r3315_daily_qc_trend'),
    supabase.rpc('founder_r3315_capa_status_board'),
    supabase.rpc('founder_r3315_root_cause_pareto'),
    supabase.rpc('founder_r3315_regulatory_impact_digest'),
    supabase.rpc('founder_r3315_high_risk_queue'),
  ]);
  ...
  const hospCols: Column<HospRow>[] = [
    ...
    { key: 'image_fail', header: 'Image Fail' },
    { key: 'leak_fail', header: 'Leak Fail' },
    { key: 'radiation_gap', header: 'Radiation Gap' },
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'check_date', header: 'Date' },
    { key: 'qc_verdict', header: 'Verdict' },
    { key: 'scope_image_quality', header: 'Image Quality' },
    { key: 'fluoroscopy_dose_rate_ok', header: 'Dose Rate' },
    { key: 'radiation_safety_ok', header: 'Radiation' },
    { key: 'scope_leak_test', header: 'Leak Test' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Customer Hospital Swallow-Study (FEES &amp; Videofluoroscopy) Equipment QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Dysphagia-lab QA log — device type &times; scope image quality &times; light source &times;
        fluoroscopy dose rate &times; frame rate (&ge;30fps) &times; recording sync &times; radiation
        safety &times; scope leak test &times; reprocessing traceability &times; calibration currency
