...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3502_qc_verdict_rollup'),
    supabase.rpc('founder_r3502_device_model_scorecard'),
    supabase.rpc('founder_r3502_parameter_verdict_matrix'),
    supabase.rpc('founder_r3502_monthly_accuracy_trend'),
    supabase.rpc('founder_r3502_capa_status_board'),
    supabase.rpc('founder_r3502_root_cause_pareto'),
    supabase.rpc('founder_r3502_accuracy_impact_digest'),
    supabase.rpc('founder_r3502_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Blood Collection Monitor / Donor Weigh-Mixer QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital blood-bank collection monitor &amp; donor weigh-mixer QA log — parameter (target
        &amp; measured volume mL, weight accuracy g, mix cycles/min, flow rate mL/min, clamp response
        sec) &times; device model &times; department &times; reference vs measured value &times;
        deviation % against tolerance band &times; alarm function &times; calibration currency &amp;
        CAPA closure. Founder-gated view: QC verdicts, device-model scorecards, parameter &times;
        verdict matrix, monthly accuracy trend, root-cause pareto, and accuracy-impact digest across
        NABH, CDSCO &amp; ISO 15189 surfaces.
      </p>
