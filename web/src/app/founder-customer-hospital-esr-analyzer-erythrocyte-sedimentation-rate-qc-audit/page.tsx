...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3523_qc_verdict_rollup'),
    supabase.rpc('founder_r3523_device_model_scorecard'),
    supabase.rpc('founder_r3523_parameter_verdict_matrix'),
    supabase.rpc('founder_r3523_monthly_accuracy_trend'),
    supabase.rpc('founder_r3523_capa_status_board'),
    supabase.rpc('founder_r3523_root_cause_pareto'),
    supabase.rpc('founder_r3523_accuracy_impact_digest'),
    supabase.rpc('founder_r3523_high_risk_queue'),
  ]);
  ...
  const digestCols: Column<DigestRow>[] = [
    ...
    { key: 'checks', header: 'Checks' },
    { key: 'out_of_tolerance', header: 'Out of Tolerance' },
    { key: 'in_tolerance_pct', header: 'In-Tolerance %' },
    { key: 'avg_deviation_pct', header: 'Avg Deviation %' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        ESR Analyzer (Erythrocyte-Sedimentation-Rate) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Automated ESR analyzer QA log — parameter (mm/hr accuracy, incubation temperature, optical
        alignment, carryover, repeatability CV &amp; sample volume) &times; device model &times;
        reference vs measured value &times; deviation % &times; within-tolerance flag &times;
        calibration date &amp; CAPA closure. Founder-gated view: QC verdicts, device-model scorecards,
        parameter &times; verdict matrix, root-cause pareto, and accuracy-impact digest across NABH
        &amp; ISO 15189 surfaces.
      </p>
