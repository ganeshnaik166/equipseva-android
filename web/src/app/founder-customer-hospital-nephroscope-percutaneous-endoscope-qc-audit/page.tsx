...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3555_qc_verdict_rollup'),
    supabase.rpc('founder_r3555_device_model_scorecard'),
    supabase.rpc('founder_r3555_parameter_verdict_matrix'),
    supabase.rpc('founder_r3555_monthly_calibration_trend'),
    supabase.rpc('founder_r3555_capa_status_board'),
    supabase.rpc('founder_r3555_root_cause_pareto'),
    supabase.rpc('founder_r3555_accuracy_impact_digest'),
    supabase.rpc('founder_r3555_high_risk_queue'),
  ]);
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'parameter', header: 'Parameter' },
    { key: 'reference_value', header: 'Ref' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Nephroscope (Percutaneous Endoscope) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital nephroscope (percutaneous renal endoscope) QC log — device model &times; parameter
        (image resolution, light transmission, irrigation channel flow ml/min, viewing angle,
        color fidelity &amp; seal-leak test) &times; reference vs measured &times; deviation %
