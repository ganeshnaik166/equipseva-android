...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3571_qc_verdict_rollup'),
    supabase.rpc('founder_r3571_device_model_scorecard'),
    supabase.rpc('founder_r3571_parameter_verdict_matrix'),
    supabase.rpc('founder_r3571_monthly_accuracy_trend'),
    supabase.rpc('founder_r3571_capa_status_board'),
    supabase.rpc('founder_r3571_root_cause_pareto'),
    supabase.rpc('founder_r3571_accuracy_impact_digest'),
    supabase.rpc('founder_r3571_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Video-EEG Telemetry / Epilepsy-Monitoring-Unit (EMU) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Video-EEG telemetry / epilepsy-monitoring-unit QA log — parameter (channel count, video-EEG
        sync offset ms, electrode impedance kOhm, sampling rate Hz, baseline noise &micro;V, recording
        uptime %) &times; reference vs measured &times; deviation % &times; within-tolerance &times;
        QC verdict &amp; CAPA closure. Founder-gated view: verdict distribution, device-model
        scorecards, parameter &times; verdict matrix, monthly accuracy trend, root-cause pareto, and
