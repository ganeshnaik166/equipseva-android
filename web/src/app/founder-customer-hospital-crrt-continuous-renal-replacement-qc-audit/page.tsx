...
type RiskRow = {
  ...
  check_date: string;
  qc_verdict: string;
  pressure_sensor_ok: string | null;
...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3402_qc_verdict_rollup'),
    supabase.rpc('founder_r3402_hospital_scorecard'),
    supabase.rpc('founder_r3402_machine_type_unit_matrix'),
    supabase.rpc('founder_r3402_daily_qc_trend'),
    supabase.rpc('founder_r3402_capa_status_board'),
    supabase.rpc('founder_r3402_root_cause_pareto'),
    supabase.rpc('founder_r3402_regulatory_impact_digest'),
    supabase.rpc('founder_r3402_high_risk_queue'),
  ]);
  ...
  const hospCols: Column<HospRow>[] = [
    ...
    { key: 'sensor_fail', header: 'Sensor Fail' },
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'check_date', header: 'Date' },
    { key: 'qc_verdict', header: 'Verdict' },
    { key: 'pressure_sensor_ok', header: 'Pressure Sensor' },
    { key: 'air_detector_test', header: 'Air Detector' },
    { key: 'disposable_set_stock', header: 'Stock' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Customer Hospital CRRT Continuous Renal Replacement QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        ICU acute-dialysis CRRT machine QA log — machine type (CVVH, CVVHD, CVVHDF, SLED &amp;
        hybrid CRRT) &times; ICU unit &times; blood-pump flow accuracy % &times; fluid-balance
        accuracy ml &times; scale calibration &times; pressure sensor &times; air detector &times;
        blood-leak detector &times; warmer temp &times; anticoagulation pump &times; alarm/battery
        &times; disposable-set stock &times; calibration currency &amp; CAPA closure. Founder-gated
      ...
      <section style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.2rem', marginBottom: '0.5rem' }}>3. Machine type &times; ICU unit matrix</h2>
        <DataTable
          rows={matrixRows}
          columns={matrixCols}
          emptyMessage="No checks by machine type."
          rowKey={(r, i) => `${r.machine_type}-${r.icu_unit}-${i}`}
        />
      </section>
