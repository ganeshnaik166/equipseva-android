...
export const metadata = { title: "Monthly KPI snapshot — EquipSeva Founder Console" };
...
export default async function MonthlyKpiSnapshotPage() {
  await requireFounder();
  const supabase = await getSupabaseServerClient();
  const { data, error } = await supabase.rpc("founder_monthly_kpi_snapshot");
  if (error) throw new Error(`founder_monthly_kpi_snapshot: ${error.message}`);
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-baseline justify-between">
        <h1 className="text-xl font-semibold">Monthly KPI snapshot (12mo)</h1>
        <span className="text-xs text-[var(--color-muted)]">8 raw KPIs per month · time series</span>
      </header>
