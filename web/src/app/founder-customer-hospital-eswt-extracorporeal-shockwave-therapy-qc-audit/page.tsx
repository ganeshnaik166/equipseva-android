...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3590_qc_verdict_rollup'),
    supabase.rpc('founder_r3590_device_model_scorecard'),
    supabase.rpc('founder_r3590_parameter_verdict_matrix'),
    supabase.rpc('founder_r3590_monthly_calibration_trend'),
    supabase.rpc('founder_r3590_capa_status_board'),
    supabase.rpc('founder_r3590_root_cause_pareto'),
    supabase.rpc('founder_r3590_accuracy_impact_digest'),
    supabase.rpc('founder_r3590_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        ESWT Extracorporeal Shockwave Therapy QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital ESWT (orthopedic &amp; urology, non-lithotripsy) shockwave-therapy QA log — device
        model &times; parameter (energy flux density mJ/mm&sup2;, pulse frequency Hz, pressure bar,
        shock-count accuracy, probe-output stability, focal depth mm) &times; reference vs measured
        &times; deviation % &times; tolerance &times; calibration currency &times; verdict &amp; CAPA
        closure. Founder-gated view: QC verdicts, device-model scorecards, parameter matrices,
