...
type ScorecardRow = {
  ...
  avg_battery_health_pct: number | null;
  transducer_faults: number;
  pass_pct: number;
};
type MatrixRow = {
  ward_or_dept: string;
...
type RiskRow = {
  ...
  measured_volume_ml: number | null;
  battery_health_pct: number | null;
  transducer_status: string | null;
  notes: string | null;
};
...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3427_qc_verdict_rollup'),
    supabase.rpc('founder_r3427_device_model_scorecard'),
    supabase.rpc('founder_r3427_ward_verdict_matrix'),
    supabase.rpc('founder_r3427_monthly_calibration_trend'),
    supabase.rpc('founder_r3427_capa_status_board'),
    supabase.rpc('founder_r3427_root_cause_pareto'),
    supabase.rpc('founder_r3427_accuracy_impact_digest'),
    supabase.rpc('founder_r3427_high_risk_queue'),
  ]);
  ...
  const scorecardCols: Column<ScorecardRow>[] = [
    ...
    { key: 'avg_battery_health_pct', header: 'Avg Battery %' },
    { key: 'transducer_faults', header: 'Transducer Faults' },
  ...
  const impactCols: Column<ImpactRow>[] = [
    { key: 'tolerance_band', header: 'Tolerance Band' },
    { key: 'checks', header: 'Checks' },
    { key: 'passed', header: 'Passed' },
    { key: 'failed', header: 'Failed' },
    { key: 'avg_deviation_pct', header: 'Avg Deviation %' },
    { key: 'max_abs_deviation_pct', header: 'Max Abs Deviation %' },
  ];
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'measured_volume_ml', header: 'Measured mL' },
    { key: 'battery_health_pct', header: 'Battery %' },
    { key: 'transducer_status', header: 'Transducer' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Bladder Scanner Ultrasound Bladder-Volume QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Portable bladder-scanner (ultrasound post-void-residual volume) QA log — device model (Verathon
        BVI 9400, BVI 6100, Prime Plus, Caresono PadScan HD5, Mcube Biocon-700) &times; ward &times;
        phantom vs measured volume mL &times; deviation % &times; probe frequency MHz &times; scan plane
        &times; battery health &times; transducer condition &times; calibration currency &amp; CAPA closure.
        Founder-gated view: QC verdicts, device-model scorecards, root-cause pareto, and accuracy-impact
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        <DataTable
          rows={impactRows}
          columns={impactCols}
          emptyMessage="No accuracy-impact data."
          rowKey={(r, i) => String(r.tolerance_band ?? i)}
        />
      </section>
