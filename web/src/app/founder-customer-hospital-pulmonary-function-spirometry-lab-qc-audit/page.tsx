...
type MatrixRow = {
  ...
  checks: number;
  passed: number;
  avg_volume_cal_error_pct: number;
  fail_checks: number;
};
type TrendRow = {
  check_date: string;
...
type RiskRow = {
  ...
  qc_verdict: string;
  leak_test: string;
  volume_calibration_error_pct: number | null;
  notes: string | null;
};
...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3271_qc_verdict_rollup'),
    supabase.rpc('founder_r3271_hospital_scorecard'),
    supabase.rpc('founder_r3271_device_department_matrix'),
    supabase.rpc('founder_r3271_daily_qc_trend'),
    supabase.rpc('founder_r3271_capa_status_board'),
    supabase.rpc('founder_r3271_root_cause_pareto'),
    supabase.rpc('founder_r3271_regulatory_impact_digest'),
    supabase.rpc('founder_r3271_high_risk_queue'),
  ]);
  ...
  const hospCols: Column<HospRow>[] = [
    ...
    { key: 'bio_control_fail', header: 'Bio-Control Fail' },
    { key: 'btps_fail', header: 'BTPS Fail' },
  ...
  const matrixCols: Column<MatrixRow>[] = [
    ...
    { key: 'checks', header: 'Checks' },
    { key: 'passed', header: 'Passed' },
    { key: 'avg_volume_cal_error_pct', header: 'Avg Vol-Cal Err %' },
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'check_date', header: 'Date' },
    { key: 'qc_verdict', header: 'Verdict' },
    { key: 'leak_test', header: 'Leak Test' },
    { key: 'volume_calibration_error_pct', header: 'Vol-Cal Err %' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Customer Hospital Pulmonary Function (PFT) Lab Equipment QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        PFT lab QA log — device type &times; 3L-syringe volume calibration &times; flow linearity
        &times; leak test &times; biological-control subject &times; DLCO gas-analyzer calibration
        &times; ambient BTPS correction &times; inline-filter hygiene &times; predicted-equation
        currency &amp; CAPA closure. Founder-gated view: QC verdicts, hospital scorecards,
        root-cause pareto, and regulatory-impact digest across NABH &amp; ISO 15189 surfaces.
      </p>
