...
type RiskRow = {
  ...
  channel_leak_pass: boolean | null;
  notes: string | null;
};
...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3567_qc_verdict_rollup'),
    supabase.rpc('founder_r3567_device_model_scorecard'),
    supabase.rpc('founder_r3567_parameter_verdict_matrix'),
    supabase.rpc('founder_r3567_monthly_accuracy_trend'),
    supabase.rpc('founder_r3567_capa_status_board'),
    supabase.rpc('founder_r3567_root_cause_pareto'),
    supabase.rpc('founder_r3567_accuracy_impact_digest'),
    supabase.rpc('founder_r3567_high_risk_queue'),
  ]);
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'channel_leak_pass', header: 'Leak Pass' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Mediastinoscope Endoscope QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital mediastinoscope endoscope imaging QC log — parameter (image resolution, light
        transmission, viewing angle, colour fidelity, focus clarity &amp; channel seal-leak test)
        &times; device model &times; reference vs measured value &times; deviation % &times;
        tolerance &times; channel leak &times; light output &times; calibration date &amp; CAPA
