...
type HospRow = {
  ...
  conditional: number;
  failed: number;
  cell_count_issues: number;
  pachymetry_issues: number;
...
type MatrixRow = {
  ...
  checks: number;
  passed: number;
  avg_pachymetry_error_um: number | null;
  failed: number;
};
type TrendRow = {
  check_date: string;
...
type RiskRow = {
  ...
  check_date: string;
  qc_verdict: string;
  cell_count_accuracy_ok: string | null;
  probe_tip_condition: string | null;
  pachymetry_accuracy_error_um: number | null;
  notes: string | null;
};
...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3359_qc_verdict_rollup'),
    supabase.rpc('founder_r3359_hospital_scorecard'),
    supabase.rpc('founder_r3359_device_department_matrix'),
    supabase.rpc('founder_r3359_daily_qc_trend'),
    supabase.rpc('founder_r3359_capa_status_board'),
    supabase.rpc('founder_r3359_root_cause_pareto'),
    supabase.rpc('founder_r3359_regulatory_impact_digest'),
    supabase.rpc('founder_r3359_high_risk_queue'),
  ]);
  ...
  const hospCols: Column<HospRow>[] = [
    ...
    { key: 'cell_count_issues', header: 'Cell-Count Issues' },
    { key: 'pachymetry_issues', header: 'Pachymetry Issues' },
  ...
  const matrixCols: Column<MatrixRow>[] = [
    ...
    { key: 'checks', header: 'Checks' },
    { key: 'passed', header: 'Passed' },
    { key: 'avg_pachymetry_error_um', header: 'Avg Pachymetry Err um' },
    { key: 'failed', header: 'Failed' },
  ];
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'check_date', header: 'Date' },
    { key: 'qc_verdict', header: 'Verdict' },
    { key: 'cell_count_accuracy_ok', header: 'Cell-Count' },
    { key: 'probe_tip_condition', header: 'Probe Tip' },
    { key: 'pachymetry_accuracy_error_um', header: 'Pachymetry Err um' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Customer Hospital Ophthalmic Anterior-Segment Metrology QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Anterior-segment metrology QA log — device type &times; model-eye calibration &times;
        endothelial cell-count accuracy &times; corneal pachymetry error um &times; probe-tip
        condition &times; image focus &times; illumination &times; measurement repeatability &times;
        software norms &times; calibration currency &amp; CAPA closure. Founder-gated view: QC
