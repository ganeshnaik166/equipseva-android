...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3570_qc_verdict_rollup'),
    supabase.rpc('founder_r3570_device_model_scorecard'),
    supabase.rpc('founder_r3570_parameter_verdict_matrix'),
    supabase.rpc('founder_r3570_monthly_calibration_trend'),
    supabase.rpc('founder_r3570_capa_status_board'),
    supabase.rpc('founder_r3570_root_cause_pareto'),
    supabase.rpc('founder_r3570_accuracy_impact_digest'),
    supabase.rpc('founder_r3570_high_risk_queue'),
  ]);
  ...
  const modelCols: Column<ModelRow>[] = [
    ...
    { key: 'avg_deviation_pct', header: 'Avg Abs Deviation %' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Controlled-Rate Freezer (Cryopreservation) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Cryopreservation controlled-rate freezer QA log for stem-cell, cord-blood &amp; embryo
        programmes — device model &times; parameter (cooling rate &deg;C/min, chamber temp,
        LN2 consumption, uniformity delta, seeding temp, probe accuracy) &times; reference vs
        measured &times; deviation % &times; within-tolerance &times; calibration date &times;
        verdict &amp; CAPA closure. Founder-gated view: QC verdicts, device-model scorecards,
        parameter &times; verdict matrix, accuracy-impact digest, root-cause pareto, and CAPA
        status across NABH &amp; CDSCO surfaces.
      </p>
