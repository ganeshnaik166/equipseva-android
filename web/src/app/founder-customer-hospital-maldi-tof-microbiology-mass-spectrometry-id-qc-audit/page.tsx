...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3470_qc_verdict_rollup'),
    supabase.rpc('founder_r3470_device_model_scorecard'),
    supabase.rpc('founder_r3470_parameter_verdict_matrix'),
    supabase.rpc('founder_r3470_monthly_accuracy_trend'),
    supabase.rpc('founder_r3470_capa_status_board'),
    supabase.rpc('founder_r3470_root_cause_pareto'),
    supabase.rpc('founder_r3470_accuracy_impact_digest'),
    supabase.rpc('founder_r3470_high_risk_queue'),
  ]);
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'spectra_quality', header: 'Spectra' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        MALDI-TOF Microbiology Mass-Spectrometry (Organism ID) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        MALDI-TOF microbiology mass-spectrometry organism-ID QA log — QC parameter (mass accuracy ppm,
        laser energy %, resolution FWHM, calibrant score, source vacuum, ID confidence %) &times; device
        model (Bruker Biotyper Sirius, bioMerieux VITEK MS &amp; MS PRIME, Autobio Autof MS1000, Zybio
        EXS2600) &times; reference vs measured value &times; deviation &times; tolerance &times;
        calibration currency &amp; CAPA closure. Founder-gated view: QC verdicts, device-model
        scorecards, root-cause pareto, and accuracy-impact digest across NABL &amp; CDSCO surfaces.
      </p>
