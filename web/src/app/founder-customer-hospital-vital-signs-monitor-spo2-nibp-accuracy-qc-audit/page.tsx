...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3431_qc_verdict_rollup'),
    supabase.rpc('founder_r3431_device_model_scorecard'),
    supabase.rpc('founder_r3431_ward_verdict_matrix'),
    supabase.rpc('founder_r3431_monthly_accuracy_trend'),
    supabase.rpc('founder_r3431_capa_status_board'),
    supabase.rpc('founder_r3431_root_cause_pareto'),
    supabase.rpc('founder_r3431_accuracy_impact_digest'),
    supabase.rpc('founder_r3431_high_risk_queue'),
  ]);
  ...
  const modelCols: Column<ModelRow>[] = [
    ...
    { key: 'alarm_failures', header: 'Alarm Failures' },
  ...
  const digestCols: Column<DigestRow>[] = [
    ...
    { key: 'checks', header: 'Checks' },
    { key: 'avg_reference', header: 'Avg Reference' },
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'qc_verdict', header: 'Verdict' },
    { key: 'alarm_test_ok', header: 'Alarm OK' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Vital-Signs Monitor SpO2 / NIBP Accuracy QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Multipara vital-signs monitor accuracy QA log — parameter (SpO2, NIBP systolic &amp;
        diastolic, pulse rate, respiration rate, temperature) &times; reference vs measured value
        &times; deviation % &times; simulator model &times; within-tolerance &times; alarm test
        &times; calibration date &amp; CAPA closure. Founder-gated view: QC verdicts, device-model
        scorecards, ward &times; verdict matrix, monthly accuracy trend, root-cause pareto, and the
        out-of-tolerance high-risk queue across NABH &amp; CDSCO surfaces.
      </p>
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        <DataTable
          rows={verdictRows}
          columns={verdictCols}
          emptyMessage="No accuracy QC checks logged yet."
