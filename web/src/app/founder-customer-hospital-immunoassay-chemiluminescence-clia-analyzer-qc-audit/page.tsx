...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3466_qc_verdict_rollup'),
    supabase.rpc('founder_r3466_device_model_scorecard'),
    supabase.rpc('founder_r3466_parameter_verdict_matrix'),
    supabase.rpc('founder_r3466_monthly_calibration_trend'),
    supabase.rpc('founder_r3466_capa_status_board'),
    supabase.rpc('founder_r3466_root_cause_pareto'),
    supabase.rpc('founder_r3466_accuracy_impact_digest'),
    supabase.rpc('founder_r3466_high_risk_queue'),
  ]);
  ...
  const modelCols: Column<ModelRow>[] = [
    { key: 'device_model', header: 'Analyzer Model' },
  ...
  const impactCols: Column<ImpactRow>[] = [
    { key: 'accuracy_impact', header: 'Accuracy Impact' },
    { key: 'findings', header: 'Findings' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Immunoassay Chemiluminescence (CLIA) Analyzer QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital immunoassay chemiluminescence (CLIA) analyzer QC log — parameter (TSH, troponin,
        PSA, vitamin D, RLU signal &amp; carryover %) &times; QC level (low/normal/high controls)
        &times; reference vs measured value &times; deviation % &times; tolerance &times; calibration
        date &times; verdict &amp; CAPA closure. Founder-gated view: QC verdicts, analyzer-model
        scorecards, parameter &times; verdict matrix, root-cause pareto, and accuracy-impact digest
        across NABL &amp; ISO 15189 surfaces.
      </p>
      ...
      <section style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.2rem', marginBottom: '0.5rem' }}>2. Analyzer-model QC scorecard</h2>
        <DataTable
          rows={modelRows}
          columns={modelCols}
          emptyMessage="No analyzer-model rollups."
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        <DataTable
          ...
          rowKey={(r, i) => String(r.accuracy_impact ?? i)}
        />
      </section>
