...
type MatrixRow = {
  ...
  avg_warming_temp_error_c: number;
};
type TrendRow = {
  check_date: string;
  checks: number;
...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3331_qc_verdict_rollup'),
    supabase.rpc('founder_r3331_hospital_scorecard'),
    supabase.rpc('founder_r3331_device_department_matrix'),
    supabase.rpc('founder_r3331_daily_qc_trend'),
    supabase.rpc('founder_r3331_capa_status_board'),
    supabase.rpc('founder_r3331_root_cause_pareto'),
    supabase.rpc('founder_r3331_regulatory_impact_digest'),
    supabase.rpc('founder_r3331_high_risk_queue'),
  ]);
  ...
  const hospCols: Column<HospRow>[] = [
    ...
    { key: 'wash_fail', header: 'Wash Fail' },
  ...
  const matrixCols: Column<MatrixRow>[] = [
    ...
    { key: 'avg_warming_temp_error_c', header: 'Avg Warming Err C' },
  ];
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'check_date', header: 'Date' },
    { key: 'qc_verdict', header: 'Verdict' },
    { key: 'wash_quality_ok', header: 'Wash Quality' },
    { key: 'air_detector_test', header: 'Air-Detect' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Customer Hospital Cell-Saver &amp; Rapid-Infuser Blood-Management QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Intraoperative blood-management QA log — device type &times; centrifuge bowl &times; wash
        quality &times; anticoagulant-pump accuracy &times; flow-rate accuracy % &times; air-detector
        test &times; warming-temp accuracy C &times; pressure alarm &times; disposable stock &times;
        calibration &amp; CAPA closure. Founder-gated view: QC verdicts, hospital scorecards,
