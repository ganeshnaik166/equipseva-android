...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3542_qc_verdict_rollup'),
    supabase.rpc('founder_r3542_device_model_scorecard'),
    supabase.rpc('founder_r3542_parameter_verdict_matrix'),
    supabase.rpc('founder_r3542_monthly_calibration_trend'),
    supabase.rpc('founder_r3542_capa_status_board'),
    supabase.rpc('founder_r3542_root_cause_pareto'),
    supabase.rpc('founder_r3542_accuracy_impact_digest'),
    supabase.rpc('founder_r3542_high_risk_queue'),
  ]);
  ...
  const impactCols: Column<ImpactRow>[] = [
    ...
    { key: 'checks', header: 'Checks' },
    { key: 'out_of_tolerance', header: 'Out of Tolerance' },
    { key: 'fail_checks', header: 'Fail Checks' },
    { key: 'avg_deviation_pct', header: 'Avg Deviation %' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        EBUS Endobronchial-Ultrasound Bronchoscope QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital EBUS (endobronchial ultrasound) bronchoscope QA log — parameter (ultrasound frequency
        MHz, image resolution, tip angulation degrees, light output lux, channel seal leak &amp; doppler
        sensitivity) &times; reference vs measured value &times; deviation % &times; within-tolerance
        &times; calibration date &times; QC verdict &amp; CAPA closure. Founder-gated view: QC verdicts,
        device-model scorecards, parameter &times; verdict matrix, monthly calibration/accuracy trend,
        root-cause pareto, accuracy-impact digest and the out-of-tolerance / seal-leak high-risk queue.
      </p>
