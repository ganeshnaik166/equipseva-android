...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3559_qc_verdict_rollup'),
    supabase.rpc('founder_r3559_device_model_scorecard'),
    supabase.rpc('founder_r3559_parameter_verdict_matrix'),
    supabase.rpc('founder_r3559_monthly_calibration_trend'),
    supabase.rpc('founder_r3559_capa_status_board'),
    supabase.rpc('founder_r3559_root_cause_pareto'),
    supabase.rpc('founder_r3559_accuracy_impact_digest'),
    supabase.rpc('founder_r3559_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Platelet-Function Aggregometer QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital platelet-function analyzer / aggregometer QC log — device model (ChronoLog,
        Helena AggRAM, Bio/Data PAP-8E, Sysmex) &times; channel &times; agonist (ADP, collagen,
        arachidonic acid, epinephrine, ristocetin, thrombin) &times; parameter (aggregation %
        accuracy, optical baseline, stir speed, channel temperature, response time, carryover)
        &times; reference vs measured &times; deviation % &times; tolerance &amp; calibration
        currency, with CAPA closure. Founder-gated view: QC verdicts, device-model scorecards,
        root-cause pareto, and accuracy-impact digest across NABL &amp; CDSCO surfaces.
      </p>
