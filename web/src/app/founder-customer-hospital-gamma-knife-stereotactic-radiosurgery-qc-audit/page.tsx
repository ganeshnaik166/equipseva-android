...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3582_qc_verdict_rollup'),
    supabase.rpc('founder_r3582_device_model_scorecard'),
    supabase.rpc('founder_r3582_parameter_verdict_matrix'),
    supabase.rpc('founder_r3582_monthly_calibration_trend'),
    supabase.rpc('founder_r3582_capa_status_board'),
    supabase.rpc('founder_r3582_root_cause_pareto'),
    supabase.rpc('founder_r3582_accuracy_impact_digest'),
    supabase.rpc('founder_r3582_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Gamma Knife Stereotactic Radiosurgery QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Gamma Knife SRS QA log — parameter (Co-60 source output Gy/min, dose-rate accuracy,
        positioning accuracy mm, timer accuracy, radiation/mechanical coincidence mm &amp; helmet
        sector dose uniformity) &times; device model &times; reference vs measured &times; deviation %
        &times; within-tolerance &times; calibration date &times; QC verdict &amp; CAPA closure.
        Founder-gated view: QC verdicts, device-model scorecards, root-cause pareto, and
        accuracy-impact digest across AERB, NABH &amp; CDSCO surfaces.
      </p>
