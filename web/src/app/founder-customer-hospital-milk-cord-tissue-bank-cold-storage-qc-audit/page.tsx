...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3282_qc_verdict_rollup'),
    supabase.rpc('founder_r3282_hospital_scorecard'),
    supabase.rpc('founder_r3282_device_bank_matrix'),
    supabase.rpc('founder_r3282_daily_qc_trend'),
    supabase.rpc('founder_r3282_capa_status_board'),
    supabase.rpc('founder_r3282_root_cause_pareto'),
    supabase.rpc('founder_r3282_regulatory_impact_digest'),
    supabase.rpc('founder_r3282_high_risk_queue'),
  ]);
  ...
  const matrixCols: Column<MatrixRow>[] = [
    ...
    { key: 'bank_type', header: 'Bank Type' },
    { key: 'audits', header: 'Checks' },
    { key: 'passed', header: 'Passed' },
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'bank_type', header: 'Bank' },
    { key: 'check_date', header: 'Date' },
    { key: 'qc_verdict', header: 'Verdict' },
    { key: 'temp_in_range', header: 'Temp In Range' },
    { key: 'alarm_test', header: 'Alarm Test' },
    { key: 'ln2_level_pct', header: 'LN2 %' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Customer Hospital Milk-Bank / Cord-Blood / Tissue-Bank Cold-Storage &amp; Processing QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Cold-chain QA log — device type &times; bank type &times; temperature-in-range &times;
        pasteurization Holder 62.5C/30min cycle &times; LN2 level % &times; alarm test &times;
        chart recorder &times; backup power &times; contamination control &times; calibration currency
      ...
      <section style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.2rem', marginBottom: '0.5rem' }}>3. Device type &times; bank type matrix</h2>
        <DataTable
          rows={matrixRows}
          columns={matrixCols}
          emptyMessage="No checks by device."
          rowKey={(r, i) => `${r.device_type}-${r.bank_type}-${i}`}
        />
      </section>
