...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3510_qc_verdict_rollup'),
    supabase.rpc('founder_r3510_device_model_scorecard'),
    supabase.rpc('founder_r3510_parameter_verdict_matrix'),
    supabase.rpc('founder_r3510_monthly_calibration_trend'),
    supabase.rpc('founder_r3510_capa_status_board'),
    supabase.rpc('founder_r3510_root_cause_pareto'),
    supabase.rpc('founder_r3510_accuracy_impact_digest'),
    supabase.rpc('founder_r3510_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Dermatome / Skin-Graft Mesher QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital dermatome &amp; skin-graft mesher QA log — device model &times; parameter (blade
        oscillation RPM, set &amp; measured graft thickness, blade sharpness, mesh ratio, width
        accuracy) &times; reference vs measured value &times; deviation % &times; tolerance band
        &times; QC verdict &amp; CAPA closure. Founder-gated view: QC verdicts, device-model
        scorecards, parameter &times; verdict matrix, monthly calibration/accuracy trend,
