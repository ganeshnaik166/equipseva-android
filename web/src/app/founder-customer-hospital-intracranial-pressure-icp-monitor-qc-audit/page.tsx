...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3534_qc_verdict_rollup'),
    supabase.rpc('founder_r3534_device_model_scorecard'),
    supabase.rpc('founder_r3534_parameter_verdict_matrix'),
    supabase.rpc('founder_r3534_monthly_calibration_trend'),
    supabase.rpc('founder_r3534_capa_status_board'),
    supabase.rpc('founder_r3534_root_cause_pareto'),
    supabase.rpc('founder_r3534_accuracy_impact_digest'),
    supabase.rpc('founder_r3534_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Intracranial-Pressure (ICP) Monitor QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital ICP monitor QC log — parameter (ICP accuracy mmHg, zero drift, transducer
        sensitivity, waveform fidelity, response time &amp; temperature stability) &times; device
        model &times; reference vs measured value &times; deviation % &times; within-tolerance flag
        &times; calibration date &times; QC verdict &amp; CAPA closure. Founder-gated view: QC
        verdicts, device-model scorecards, parameter &times; verdict matrix, root-cause pareto and
        accuracy-impact digest across the ICP monitoring fleet.
      </p>
