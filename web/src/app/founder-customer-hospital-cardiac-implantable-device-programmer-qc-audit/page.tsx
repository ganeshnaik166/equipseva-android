...
type RiskRow = {
  hospital_name: string;
  programmer_code: string;
  device_type: string;
  department: string;
  check_date: string;
  qc_verdict: string;
  communication_test_pass: string;
  printer_output_ok: string;
...
export default async function Page() {
  ...
  const [verdictRes, scoreRes, matrixRes, trendRes, capaRes, causeRes, regRes, riskRes] = await Promise.all([
    supabase.rpc('founder_r3387_qc_verdict_rollup'),
    supabase.rpc('founder_r3387_hospital_scorecard'),
    supabase.rpc('founder_r3387_device_department_matrix'),
    supabase.rpc('founder_r3387_daily_qc_trend'),
    supabase.rpc('founder_r3387_capa_status_board'),
    supabase.rpc('founder_r3387_root_cause_pareto'),
    supabase.rpc('founder_r3387_regulatory_impact_digest'),
    supabase.rpc('founder_r3387_high_risk_queue'),
  ]);
  ...
  const scoreCols: Column<ScoreRow>[] = [
    ...
    { key: 'comm_fail', header: 'Comm Fail' },
  ...
  const riskCols: Column<RiskRow>[] = [
    { key: 'hospital_name', header: 'Hospital' },
    { key: 'programmer_code', header: 'Programmer' },
    { key: 'device_type', header: 'Device Type' },
    { key: 'department', header: 'Department' },
    { key: 'check_date', header: 'Date' },
    { key: 'qc_verdict', header: 'Verdict' },
    { key: 'communication_test_pass', header: 'Comm Test' },
    { key: 'printer_output_ok', header: 'Printer' },
    { key: 'emergency_pacing_function_ok', header: 'Emerg Pacing' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Cardiac Implantable-Device Programmer QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Pacemaker / ICD / CRT programmer &amp; interrogation-station QA &mdash; telemetry wand &times; comm test
        &times; battery &times; printer &times; software version &times; vendor compatibility &times; touchscreen
        &times; emergency pacing &times; data export &amp; CAPA. Founder-gated view: verdict rollup, hospital
        scorecard, device &times; department matrix, and high-risk programmer queue.
      </p>

      <section style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.2rem', marginBottom: '0.5rem' }}>1. QC verdict distribution</h2>
        <DataTable rows={verdictRows} columns={verdictCols} emptyMessage="No programmers checked yet." rowKey={(r, i) => String(r.qc_verdict ?? i)} />
      </section>
      <section style={{ marginBottom: '2rem' }}>
      ...
      <section style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.2rem', marginBottom: '0.5rem' }}>8. High-risk programmer queue</h2>
        <DataTable rows={riskRows} columns={riskCols} emptyMessage="No high-risk programmers." rowKey={(r, i) => `${r.programmer_code}-${i}`} />
      </section>
    </main>
  );
}
