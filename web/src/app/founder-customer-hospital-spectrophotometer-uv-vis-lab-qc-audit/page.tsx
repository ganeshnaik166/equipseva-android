...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3486_qc_verdict_rollup'),
    supabase.rpc('founder_r3486_model_scorecard'),
    supabase.rpc('founder_r3486_parameter_verdict_matrix'),
    supabase.rpc('founder_r3486_monthly_calibration_trend'),
    supabase.rpc('founder_r3486_capa_status_board'),
    supabase.rpc('founder_r3486_root_cause_pareto'),
    supabase.rpc('founder_r3486_accuracy_impact_digest'),
    supabase.rpc('founder_r3486_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        UV-Vis Spectrophotometer (Lab) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital laboratory UV-Vis spectrophotometer QA log — device model &times; QC parameter
        (wavelength accuracy nm, absorbance accuracy, stray light %, baseline flatness, photometric
        noise, resolution nm) &times; reference vs measured &times; deviation % &times; tolerance
        &times; calibration date &times; verdict &amp; CAPA closure. Founder-gated view: QC verdicts,
        model scorecards, parameter &times; verdict matrix, monthly accuracy trend, root-cause pareto,
        and accuracy-impact digest across NABL, NABH &amp; ISO 15189 surfaces.
      </p>
