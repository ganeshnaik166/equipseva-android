...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3467_qc_verdict_rollup'),
    supabase.rpc('founder_r3467_device_model_scorecard'),
    supabase.rpc('founder_r3467_parameter_verdict_matrix'),
    supabase.rpc('founder_r3467_monthly_calibration_trend'),
    supabase.rpc('founder_r3467_capa_status_board'),
    supabase.rpc('founder_r3467_root_cause_pareto'),
    supabase.rpc('founder_r3467_accuracy_impact_digest'),
    supabase.rpc('founder_r3467_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        HbA1c / Glycated-Hemoglobin Diabetes Analyzer QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital HbA1c / glycated-hemoglobin analyzer (HPLC &amp; boronate-affinity) QC log for diabetes
        monitoring &mdash; device model &times; parameter (HbA1c %, retention time, peak-area %CV, NGSP
        alignment, column pressure, carryover) &times; reference vs measured &times; deviation %
        &times; QC level &times; tolerance &times; calibration date &times; verdict &amp; CAPA closure.
        Founder-gated view: QC verdicts, device-model scorecards, root-cause pareto, and accuracy-impact
        digest across NABL &amp; ISO 15189 surfaces.
      </p>
