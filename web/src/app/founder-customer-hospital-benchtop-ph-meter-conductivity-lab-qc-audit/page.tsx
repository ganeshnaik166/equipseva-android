...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3487_qc_verdict_rollup'),
    supabase.rpc('founder_r3487_device_model_scorecard'),
    supabase.rpc('founder_r3487_parameter_verdict_matrix'),
    supabase.rpc('founder_r3487_monthly_calibration_trend'),
    supabase.rpc('founder_r3487_capa_status_board'),
    supabase.rpc('founder_r3487_root_cause_pareto'),
    supabase.rpc('founder_r3487_accuracy_impact_digest'),
    supabase.rpc('founder_r3487_high_risk_queue'),
  ]);
  ...
  const matrixCols: Column<MatrixRow>[] = [
    ...
    { key: 'qc_verdict', header: 'Verdict' },
    { key: 'checks', header: 'Checks' },
    { key: 'avg_deviation_pct', header: 'Avg Abs Dev %' },
  ];
  ...
  const digestCols: Column<DigestRow>[] = [
    ...
    { key: 'out_of_tolerance', header: 'Out of Tolerance' },
    { key: 'avg_abs_deviation_pct', header: 'Avg Abs Dev %' },
    { key: 'worst_deviation_pct', header: 'Worst Dev %' },
  ];
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Benchtop pH-Meter / Conductivity (Lab) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital laboratory benchtop pH-meter &amp; conductivity meter QA log — device model &times; QC
        parameter (pH slope %, offset mV, pH4 &amp; pH7 buffer checks, conductivity uS/cm,
        temperature compensation) &times; reference vs measured &times; deviation % &times; tolerance
        &times; calibration currency &times; verdict &amp; CAPA closure. Founder-gated view: QC
        verdicts, model scorecards, parameter &times; verdict matrix, monthly calibration &amp;
        accuracy trend, root-cause pareto, and accuracy-impact digest across NABL, NABH &amp; ISO 15189
        surfaces.
      </p>
