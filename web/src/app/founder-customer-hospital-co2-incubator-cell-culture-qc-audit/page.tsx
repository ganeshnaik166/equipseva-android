...
type RiskRow = {
  ...
  contamination_check: string | null;
  notes: string | null;
};
...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3482_qc_verdict_rollup'),
    supabase.rpc('founder_r3482_device_model_scorecard'),
    supabase.rpc('founder_r3482_parameter_verdict_matrix'),
    supabase.rpc('founder_r3482_monthly_calibration_trend'),
    supabase.rpc('founder_r3482_capa_status_board'),
    supabase.rpc('founder_r3482_root_cause_pareto'),
    supabase.rpc('founder_r3482_accuracy_impact_digest'),
    supabase.rpc('founder_r3482_high_risk_queue'),
  ]);
  ...
  const modelCols: Column<ModelRow>[] = [
    ...
    { key: 'contamination_flag', header: 'Contamination' },
  ...
  const accuracyCols: Column<AccuracyRow>[] = [
    { key: 'parameter', header: 'Parameter' },
    { key: 'checks', header: 'Checks' },
    { key: 'within_tolerance_count', header: 'Within Tol' },
    { key: 'out_of_tolerance_count', header: 'Out of Tol' },
    { key: 'avg_abs_deviation_pct', header: 'Avg |Deviation| %' },
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'contamination_check', header: 'Contamination' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        CO2 Incubator (Cell-Culture) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital CO2 incubator cell-culture QA log — parameter (temperature, CO2 %, humidity %,
        O2 %, CO2 recovery time &amp; in-chamber HEPA particle count) &times; reference vs measured
        &times; deviation % &times; tolerance &times; contamination guard (fungal &amp; bacterial)
        &times; door-seal &amp; HEPA integrity &times; calibration currency &amp; CAPA closure.
        Founder-gated view: QC verdicts, device-model scorecards, parameter &times; verdict matrix,
        root-cause pareto, and accuracy-impact digest across NABH, CDSCO, ISO 15189 &amp; GMP surfaces.
      </p>
