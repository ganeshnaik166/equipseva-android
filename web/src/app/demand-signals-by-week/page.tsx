...
export const metadata = { title: "Demand signals by week — EquipSeva Founder Console" };
export const dynamic = "force-dynamic";

type Row = { week_start: string; signals: number; distinct_skus: number; resolved: number };

export default async function DemandSignalsByWeekPage() {
  await requireFounder();
  const supabase = await getSupabaseServerClient();
  const { data, error } = await supabase.rpc("founder_demand_signals_by_week");
  if (error) throw new Error(`founder_demand_signals_by_week: ${error.message}`);
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-baseline justify-between">
        <h1 className="text-xl font-semibold">Demand signals by week</h1>
        <span className="text-xs text-[var(--color-muted)]">last 13 weeks · IST</span>
      </header>
      <DataTable columns={cols} rows={rows} rowKey={(r) => r.week_start} emptyMessage="No signals." />
    </div>
  );
}
