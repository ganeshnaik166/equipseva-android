...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3483_qc_verdict_rollup'),
    supabase.rpc('founder_r3483_device_model_scorecard'),
    supabase.rpc('founder_r3483_parameter_verdict_matrix'),
    supabase.rpc('founder_r3483_monthly_calibration_trend'),
    supabase.rpc('founder_r3483_capa_status_board'),
    supabase.rpc('founder_r3483_root_cause_pareto'),
    supabase.rpc('founder_r3483_accuracy_impact_digest'),
    supabase.rpc('founder_r3483_high_risk_queue'),
  ]);
  ...
  const modelCols: Column<ModelRow>[] = [
    ...
    { key: 'integrity_fail', header: 'Integrity Fail' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Cryogenic LN2 Storage-Tank Sample-Integrity QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital biobank cryogenic LN2 storage-tank QA log — device model &times; biobank zone
        (stem-cell, cord-blood, IVF embryo, tissue bank, vaccine store) &times; parameter (LN2 level %,
        vapor-phase temp &deg;C, evaporation rate L/day, low-level &amp; O2-deficiency alarms, fill-cycle
        days) &times; reference vs measured accuracy &times; deviation % &times; alarm test &times;
        sample integrity &amp; CAPA closure. Founder-gated view: QC verdicts, device-model scorecards,
