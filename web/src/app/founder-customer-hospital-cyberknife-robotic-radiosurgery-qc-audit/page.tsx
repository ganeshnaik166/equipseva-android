...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3586_qc_verdict_rollup'),
    supabase.rpc('founder_r3586_device_model_scorecard'),
    supabase.rpc('founder_r3586_parameter_verdict_matrix'),
    supabase.rpc('founder_r3586_monthly_qc_trend'),
    supabase.rpc('founder_r3586_capa_status_board'),
    supabase.rpc('founder_r3586_root_cause_pareto'),
    supabase.rpc('founder_r3586_accuracy_impact_digest'),
    supabase.rpc('founder_r3586_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        CyberKnife Robotic Radiosurgery QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital CyberKnife robotic-arm linac QA log — QC parameter (beam output cGy/MU, sub-mm
        targeting accuracy, Synchrony tracking latency, MU linearity, Winston-Lutz isocenter
        accuracy &amp; collimator field size) &times; device model &times; reference vs measured
        &times; deviation &times; tolerance band &times; CAPA closure. Founder-gated view: QC
        verdicts, device-model scorecards, parameter &times; verdict matrix, root-cause pareto, and
        accuracy-impact digest across AERB &amp; NABH surfaces.
      </p>
      ...
      <section style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.2rem', marginBottom: '0.5rem' }}>4. Monthly QC trend</h2>
        <DataTable
          rows={trendRows}
          columns={trendCols}
