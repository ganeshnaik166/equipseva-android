...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3458_qc_verdict_rollup'),
    supabase.rpc('founder_r3458_device_model_scorecard'),
    supabase.rpc('founder_r3458_parameter_verdict_matrix'),
    supabase.rpc('founder_r3458_monthly_calibration_trend'),
    supabase.rpc('founder_r3458_capa_status_board'),
    supabase.rpc('founder_r3458_root_cause_pareto'),
    supabase.rpc('founder_r3458_accuracy_impact_digest'),
    supabase.rpc('founder_r3458_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Portable / Mobile X-Ray (Digital Radiography) Detector QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Portable / mobile X-ray (digital radiography DR) QC log — device model (GE Optima XR240amx,
        Siemens Mobilett Elara, Philips MobileDiagnost wDR, Samsung GM85, Fujifilm FDR nano) &times;
        parameter (kVp accuracy, mAs linearity, output &micro;Gy, dose-area product, detector DQE,
        collimation mm) &times; reference vs measured &times; deviation % &times; tolerance band
        &times; calibration date &times; verdict &amp; CAPA closure. Founder-gated view: QC verdicts,
        device-model scorecards, root-cause pareto, and accuracy-impact digest across AERB &amp; NABH
        surfaces.
      </p>
