...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3446_qc_verdict_rollup'),
    supabase.rpc('founder_r3446_device_model_scorecard'),
    supabase.rpc('founder_r3446_parameter_verdict_matrix'),
    supabase.rpc('founder_r3446_monthly_calibration_trend'),
    supabase.rpc('founder_r3446_capa_status_board'),
    supabase.rpc('founder_r3446_root_cause_pareto'),
    supabase.rpc('founder_r3446_accuracy_impact_digest'),
    supabase.rpc('founder_r3446_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Blood / Plasma Thawer (Water-Bath) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Blood-bank blood / plasma thawer (dry or water-bath) QA log — parameter (bath temperature,
        chamber uniformity delta, thaw-cycle time, high-temp alarm, agitation rpm, probe accuracy)
        &times; reference vs measured value &times; deviation % &times; alarm test &times; calibration
        date &times; QC verdict &amp; CAPA closure. Founder-gated view: QC verdicts, device-model
        scorecards, parameter &times; verdict matrix, monthly calibration/accuracy trend, root-cause
        pareto, and out-of-tolerance / alarm-fail queue across NABH &amp; CDSCO surfaces.
      </p>
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        <DataTable
          rows={digestRows}
          columns={digestCols}
          emptyMessage="No accuracy digest data."
