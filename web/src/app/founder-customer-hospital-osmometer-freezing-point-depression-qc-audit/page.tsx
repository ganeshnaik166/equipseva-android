...
type DigestRow = {
  ...
  avg_repeatability_cv_pct: number;
};
type RiskRow = {
...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3471_qc_verdict_rollup'),
    supabase.rpc('founder_r3471_device_model_scorecard'),
    supabase.rpc('founder_r3471_parameter_verdict_matrix'),
    supabase.rpc('founder_r3471_monthly_calibration_trend'),
    supabase.rpc('founder_r3471_capa_status_board'),
    supabase.rpc('founder_r3471_root_cause_pareto'),
    supabase.rpc('founder_r3471_accuracy_impact_digest'),
    supabase.rpc('founder_r3471_high_risk_queue'),
  ]);
  ...
  const digestCols: Column<DigestRow>[] = [
    ...
    { key: 'avg_repeatability_cv_pct', header: 'Avg Repeatability CV %' },
  ];
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Osmometer (Freezing-Point Depression) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital osmometer QC log for serum &amp; urine osmolality by freezing-point depression —
        parameter (osmolality mOsm/kg, freezing point m&deg;C, cooling rate, probe calibration,
        repeatability CV, sample volume &micro;L) &times; sample type &times; reference vs measured
        &times; deviation % &times; tolerance &times; repeatability CV &times; calibration currency
        &amp; CAPA closure. Founder-gated view: QC verdicts, device-model scorecards, root-cause
        pareto, and accuracy-impact digest across NABL &amp; ISO 15189 surfaces.
      </p>
