...
type MatrixRow = { device_type: string; department: string; checks: number; passed: number; failed: number; avg_weight_error_pct: number };
type TrendRow = { check_date: string; checks: number; passed: number; failed: number; seal_issue: number; temp_issue: number };
...
type RiskRow = {
  ...
  check_date: string;
  qc_verdict: string;
  seal_test_result: string;
  temperature_control_ok: string;
  alarm_test: string;
  notes: string | null;
};
...
export default async function Page() {
  ...
  const [verdictRes, scoreRes, matrixRes, trendRes, capaRes, causeRes, regRes, riskRes] = await Promise.all([
    supabase.rpc('founder_r3395_qc_verdict_rollup'),
    supabase.rpc('founder_r3395_hospital_scorecard'),
    supabase.rpc('founder_r3395_device_department_matrix'),
    supabase.rpc('founder_r3395_daily_qc_trend'),
    supabase.rpc('founder_r3395_capa_status_board'),
    supabase.rpc('founder_r3395_root_cause_pareto'),
    supabase.rpc('founder_r3395_regulatory_impact_digest'),
    supabase.rpc('founder_r3395_high_risk_queue'),
  ]);
  ...
  const scoreCols: Column<ScoreRow>[] = [
    ...
    { key: 'seal_issue', header: 'Seal Issue' },
    { key: 'temp_issue', header: 'Temp Issue' },
  ...
  const matrixCols: Column<MatrixRow>[] = [
    ...
    { key: 'passed', header: 'Passed' },
    { key: 'failed', header: 'Failed' },
    { key: 'avg_weight_error_pct', header: 'Avg Weight Err %' },
  ];
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'check_date', header: 'Date' },
    { key: 'qc_verdict', header: 'Verdict' },
    { key: 'seal_test_result', header: 'Seal Test' },
    { key: 'temperature_control_ok', header: 'Temp Control' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Blood-Bank Tube-Sealer, Plasma-Expressor &amp; Component-Processing QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Blood-bank component-processing devices &mdash; tube sealer &times; plasma expressor &times; weigh-mixer
        &times; sterile-connection device &times; bag centrifuge &times; seal integrity &times; weight accuracy
        &times; temperature &times; sterile weld &amp; CAPA. Founder-gated view: verdict rollup, hospital scorecard,
