...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3522_qc_verdict_rollup'),
    supabase.rpc('founder_r3522_device_model_scorecard'),
    supabase.rpc('founder_r3522_parameter_verdict_matrix'),
    supabase.rpc('founder_r3522_monthly_accuracy_trend'),
    supabase.rpc('founder_r3522_capa_status_board'),
    supabase.rpc('founder_r3522_root_cause_pareto'),
    supabase.rpc('founder_r3522_accuracy_impact_digest'),
    supabase.rpc('founder_r3522_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Vascular Doppler / Ankle-Brachial-Index (ABI) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital vascular doppler / ABI unit QC log — parameter (probe frequency MHz, cuff pressure
        accuracy, signal sensitivity, ABI ratio accuracy, waveform quality, battery voltage) &times;
        reference vs measured value &times; deviation % &times; tolerance flag &times; calibration date
