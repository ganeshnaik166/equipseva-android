...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3566_qc_verdict_rollup'),
    supabase.rpc('founder_r3566_device_model_scorecard'),
    supabase.rpc('founder_r3566_parameter_verdict_matrix'),
    supabase.rpc('founder_r3566_monthly_accuracy_trend'),
    supabase.rpc('founder_r3566_capa_status_board'),
    supabase.rpc('founder_r3566_root_cause_pareto'),
    supabase.rpc('founder_r3566_accuracy_impact_digest'),
    supabase.rpc('founder_r3566_high_risk_queue'),
  ]);
  ...
  const digestCols: Column<DigestRow>[] = [
    { key: 'parameter', header: 'Parameter' },
    { key: 'checks', header: 'Checks' },
    { key: 'within_tolerance_count', header: 'In Tolerance' },
    { key: 'out_of_tolerance', header: 'Out of Tolerance' },
    { key: 'avg_deviation_pct', header: 'Avg |Dev| %' },
    { key: 'max_deviation_pct', header: 'Max |Dev| %' },
    { key: 'within_tolerance_pct', header: 'In Tolerance %' },
  ];
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Microwave-Ablation Generator QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital microwave-ablation generator QA log — device model (NeuWave Certus, Emprint HP,
        Solero MTA, AveCure MedWaves, Amica-GEN) &times; parameter (set power W, delivered power W,
        frequency GHz, reflected power %, ablation time sec, antenna temp &deg;C) &times; reference vs
        measured &times; deviation % &times; tolerance &times; antenna type &times; calibration date
        &amp; CAPA closure. Founder-gated view: QC verdicts, model scorecards, parameter &times;
        verdict matrix, monthly accuracy trend, root-cause pareto and accuracy-impact digest across
        NABH &amp; CDSCO surfaces.
      </p>
