...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3531_qc_verdict_rollup'),
    supabase.rpc('founder_r3531_device_model_scorecard'),
    supabase.rpc('founder_r3531_parameter_verdict_matrix'),
    supabase.rpc('founder_r3531_monthly_calibration_trend'),
    supabase.rpc('founder_r3531_capa_status_board'),
    supabase.rpc('founder_r3531_root_cause_pareto'),
    supabase.rpc('founder_r3531_accuracy_impact_digest'),
    supabase.rpc('founder_r3531_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Cardiac-Output Monitor (PiCCO / FloTrac) Hemodynamic QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital cardiac-output monitor hemodynamic QA log — parameter (cardiac output L/min, SVV %,
        pressure accuracy mmHg, thermistor temperature &deg;C, signal stability, zero drift) &times;
        device model (Getinge / Pulsion PiCCO, Edwards HemoSphere / EV1000 FloTrac / Vigileo) &times;
        reference vs measured &times; deviation % &times; tolerance band &times; calibration date &amp;
