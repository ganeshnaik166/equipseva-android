...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3518_qc_verdict_rollup'),
    supabase.rpc('founder_r3518_device_model_scorecard'),
    supabase.rpc('founder_r3518_parameter_verdict_matrix'),
    supabase.rpc('founder_r3518_monthly_accuracy_trend'),
    supabase.rpc('founder_r3518_capa_status_board'),
    supabase.rpc('founder_r3518_root_cause_pareto'),
    supabase.rpc('founder_r3518_accuracy_impact_digest'),
    supabase.rpc('founder_r3518_high_risk_queue'),
  ]);
  ...
  const modelCols: Column<ModelRow>[] = [
    ...
    { key: 'cal_overdue', header: 'Cal Overdue' },
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'hepa_leak_test', header: 'HEPA Leak' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Chemotherapy Compounding-Isolator / Cytotoxic-Hood QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital chemo compounding isolator &amp; cytotoxic safety cabinet QC log — cabinet type
        (Class II cytotoxic safety cabinets, negative &amp; positive compounding isolators, laminar-flow
        hoods) &times; parameter (negative pressure Pa &times; inflow &amp; downflow velocity m/s &times;
        HEPA integrity % &times; particle count &times; containment leak) &times; reference vs measured
        value &times; deviation % &times; tolerance &times; calibration currency &amp; CAPA closure.
        Founder-gated view: QC verdicts, device-model scorecards, root-cause pareto, and accuracy-impact
        digest across NABH, CDSCO &amp; USP-800 surfaces.
      </p>
