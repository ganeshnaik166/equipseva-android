...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3507_qc_verdict_rollup'),
    supabase.rpc('founder_r3507_device_model_scorecard'),
    supabase.rpc('founder_r3507_parameter_verdict_matrix'),
    supabase.rpc('founder_r3507_monthly_calibration_trend'),
    supabase.rpc('founder_r3507_capa_status_board'),
    supabase.rpc('founder_r3507_root_cause_pareto'),
    supabase.rpc('founder_r3507_accuracy_impact_digest'),
    supabase.rpc('founder_r3507_high_risk_queue'),
  ]);
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'blade_condition', header: 'Blade' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Orthopedic Cast-Saw / Plaster-Cutter QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital orthopedic oscillating cast-saw &amp; plaster-cutter QC log — parameter (oscillation
        RPM, blade wear %, dust-extraction flow, noise dB, vibration level, depth-guard mm) &times;
        device model &times; unit &times; reference vs measured value &times; deviation % &times;
        tolerance band &times; blade condition &times; calibration date &amp; CAPA closure.
