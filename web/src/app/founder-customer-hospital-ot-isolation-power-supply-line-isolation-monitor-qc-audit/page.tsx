...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3594_qc_verdict_rollup'),
    supabase.rpc('founder_r3594_device_model_scorecard'),
    supabase.rpc('founder_r3594_parameter_verdict_matrix'),
    supabase.rpc('founder_r3594_monthly_trend'),
    supabase.rpc('founder_r3594_capa_status_board'),
    supabase.rpc('founder_r3594_root_cause_pareto'),
    supabase.rpc('founder_r3594_accuracy_impact_digest'),
    supabase.rpc('founder_r3594_high_risk_queue'),
  ]);
  ...
  const digestCols: Column<DigestRow>[] = [
    ...
    { key: 'checks', header: 'Checks' },
    { key: 'out_of_tolerance', header: 'Out of Tolerance' },
    { key: 'within_tolerance_pct', header: 'Within Tolerance %' },
    { key: 'avg_deviation_pct', header: 'Avg Deviation %' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        OT Isolation Power Supply (IPS) / Line Isolation Monitor (LIM) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Operating-theatre isolated power supply (IPS) and line isolation monitor (LIM) electrical-safety
        QA log — device model &times; parameter (isolation resistance k&Omega;, line leakage current mA,
        hazard current mA, transformer load %, LIM alarm setpoint mA, ground continuity &Omega;) &times;
        reference vs measured value &times; deviation % &times; tolerance band &times; LIM alarm function
        &amp; CAPA closure. Founder-gated view: QC verdicts, device-model scorecards, root-cause pareto,
        and accuracy-impact digest across NABH, CDSCO &amp; IEC 60364-7-710 surfaces.
      </p>
