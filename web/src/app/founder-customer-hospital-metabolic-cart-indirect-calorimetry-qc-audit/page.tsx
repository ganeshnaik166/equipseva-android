...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3574_qc_verdict_rollup'),
    supabase.rpc('founder_r3574_device_model_scorecard'),
    supabase.rpc('founder_r3574_parameter_verdict_matrix'),
    supabase.rpc('founder_r3574_monthly_calibration_trend'),
    supabase.rpc('founder_r3574_capa_status_board'),
    supabase.rpc('founder_r3574_root_cause_pareto'),
    supabase.rpc('founder_r3574_accuracy_impact_digest'),
    supabase.rpc('founder_r3574_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Metabolic Cart (Indirect Calorimetry) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital metabolic cart / indirect calorimetry QA log — device model (COSMED Quark RMR, Vyaire
        Vyntus CPX, MGC Ultima CardiO2, GE Carescape, Cortex Metalyzer) &times; department &times;
        gas-analyzer type (fuel cell, paramagnetic, zirconia, infrared CO2) &times; parameter (VO2 &amp;
        VCO2 accuracy, flow accuracy L/min, O2 &amp; CO2 sensor drift, RQ accuracy) &times; reference vs
        measured &times; deviation % vs tolerance &times; calibration currency &amp; CAPA closure.
