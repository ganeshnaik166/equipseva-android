...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3474_qc_verdict_rollup'),
    supabase.rpc('founder_r3474_device_model_scorecard'),
    supabase.rpc('founder_r3474_parameter_verdict_matrix'),
    supabase.rpc('founder_r3474_monthly_accuracy_trend'),
    supabase.rpc('founder_r3474_capa_status_board'),
    supabase.rpc('founder_r3474_root_cause_pareto'),
    supabase.rpc('founder_r3474_accuracy_impact_digest'),
    supabase.rpc('founder_r3474_high_risk_queue'),
  ]);
  ...
  const matrixCols: Column<MatrixRow>[] = [
    ...
    { key: 'avg_measured_value', header: 'Avg Measured' },
  ];
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'compressor_status', header: 'Compressor' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Ultra-Low-Temperature (-80C) Freezer QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital ultra-low-temperature (-80C) freezer QA log — parameter (setpoint temp, chamber
        uniformity delta, door-open recovery, high-temp alarm, compressor current, battery backup)
        &times; device model &times; location &times; reference vs measured accuracy &times; deviation
        percent &times; alarm, door-seal &amp; battery integrity &times; calibration currency &amp; CAPA
        closure. Founder-gated view: QC verdicts, device-model scorecards, root-cause pareto, and an
        accuracy-impact digest across NABH, CDSCO &amp; NABL cold-chain surfaces.
      </p>
