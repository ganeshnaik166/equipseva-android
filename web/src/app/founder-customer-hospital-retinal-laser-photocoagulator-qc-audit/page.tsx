...
type MatrixRow = { device_type: string; department: string; checks: number; passed: number; failed: number; avg_power_error_pct: number };
type TrendRow = { check_date: string; checks: number; passed: number; failed: number; power_issue: number; safety_issue: number };
...
type RiskRow = {
  ...
  fiber_delivery_ok: string;
  pattern_alignment_ok: string;
  notes: string | null;
};
...
export default async function Page() {
  ...
  const [verdictRes, scoreRes, matrixRes, trendRes, capaRes, causeRes, regRes, riskRes] = await Promise.all([
    supabase.rpc('founder_r3398_qc_verdict_rollup'),
    supabase.rpc('founder_r3398_hospital_scorecard'),
    supabase.rpc('founder_r3398_device_department_matrix'),
    supabase.rpc('founder_r3398_daily_qc_trend'),
    supabase.rpc('founder_r3398_capa_status_board'),
    supabase.rpc('founder_r3398_root_cause_pareto'),
    supabase.rpc('founder_r3398_regulatory_impact_digest'),
    supabase.rpc('founder_r3398_high_risk_queue'),
  ]);
  ...
  const scoreCols: Column<ScoreRow>[] = [
    ...
    { key: 'power_issue', header: 'Power Issue' },
    { key: 'safety_issue', header: 'Safety Issue' },
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'fiber_delivery_ok', header: 'Fiber' },
    { key: 'pattern_alignment_ok', header: 'Pattern' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Retinal-Laser Photocoagulator &amp; Ophthalmic Treatment-Laser QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        532/577nm photocoagulators, multi-spot pattern, diode &amp; micropulse lasers &mdash; power output
        &times; aiming beam &times; spot size &times; pattern alignment &times; fiber delivery &times; footswitch
        &times; safety filter &times; eyewear &amp; CAPA. Founder-gated view: verdict rollup, hospital scorecard,
        device &times; department matrix, and high-risk laser queue.
      </p>

      <section style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.2rem', marginBottom: '0.5rem' }}>1. QC verdict distribution</h2>
        <DataTable rows={verdictRows} columns={verdictCols} emptyMessage="No lasers checked yet." rowKey={(r, i) => String(r.qc_verdict ?? i)} />
      </section>
      <section style={{ marginBottom: '2rem' }}>
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        <DataTable rows={riskRows} columns={riskCols} emptyMessage="No high-risk lasers." rowKey={(r, i) => `${r.device_code}-${i}`} />
      </section>
    </main>
  );
}
