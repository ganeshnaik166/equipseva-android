...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3530_qc_verdict_rollup'),
    supabase.rpc('founder_r3530_device_model_scorecard'),
    supabase.rpc('founder_r3530_parameter_verdict_matrix'),
    supabase.rpc('founder_r3530_monthly_accuracy_trend'),
    supabase.rpc('founder_r3530_capa_status_board'),
    supabase.rpc('founder_r3530_root_cause_pareto'),
    supabase.rpc('founder_r3530_accuracy_impact_digest'),
    supabase.rpc('founder_r3530_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Vein-Finder / Vein-Viewer (Infrared) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Infrared / transillumination vein-finder &amp; vein-viewer QC log — device model &times;
        parameter (IR light output, image contrast, projection alignment mm, depth penetration mm,
        battery runtime min, resolution) &times; reference vs measured value &times; deviation %
        &times; tolerance &times; calibration date &times; verdict &amp; CAPA closure. Founder-gated
        view: QC verdicts, device-model scorecards, parameter &times; verdict matrix, accuracy-impact
        digest, root-cause pareto, and regulatory-impact CAPA board across NABH &amp; CDSCO surfaces.
      </p>
