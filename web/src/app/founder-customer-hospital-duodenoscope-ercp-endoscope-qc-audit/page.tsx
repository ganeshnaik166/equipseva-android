...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3547_qc_verdict_rollup'),
    supabase.rpc('founder_r3547_device_model_scorecard'),
    supabase.rpc('founder_r3547_parameter_verdict_matrix'),
    supabase.rpc('founder_r3547_monthly_calibration_trend'),
    supabase.rpc('founder_r3547_capa_status_board'),
    supabase.rpc('founder_r3547_root_cause_pareto'),
    supabase.rpc('founder_r3547_accuracy_impact_digest'),
    supabase.rpc('founder_r3547_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Duodenoscope (ERCP) Endoscope QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital duodenoscope (ERCP) endoscope QC log — parameter (image resolution, elevator
        movement, light output, channel seal leak, angulation &amp; distal-cap integrity) &times;
        device model &times; reference-vs-measured accuracy &times; deviation % &times; within-tolerance
        &times; reprocess leak test &amp; CAPA closure. Founder-gated view: QC verdicts, device-model
