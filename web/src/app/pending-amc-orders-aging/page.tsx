...
export const metadata = { title: "Pending AMC orders aging — EquipSeva Founder Console" };
export const dynamic = "force-dynamic";

type Row = { bucket: string; cnt: number; rupees_sum: number; oldest_hours: number };

export default async function PendingAmcOrdersAgingPage() {
  await requireFounder();
  const supabase = await getSupabaseServerClient();
  const { data, error } = await supabase.rpc("founder_pending_amc_orders_aging");
  if (error) throw new Error(`founder_pending_amc_orders_aging: ${error.message}`);
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-baseline justify-between">
        <h1 className="text-xl font-semibold">Pending AMC orders aging</h1>
        <span className="text-xs text-[var(--color-muted)]">amc_payment_orders.status='pending' age distribution</span>
      </header>
      <DataTable columns={cols} rows={rows} rowKey={(r) => r.bucket} emptyMessage="No pending orders." />
      <section className="rounded border border-[var(--color-border)] bg-white p-3 text-xs text-[var(--color-muted)]">
        Orders pending &gt;24h likely abandoned. Check Razorpay dashboard for stuck checkouts.
      </section>
    </div>
  );
}
