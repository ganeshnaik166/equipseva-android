...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3578_qc_verdict_rollup'),
    supabase.rpc('founder_r3578_device_model_scorecard'),
    supabase.rpc('founder_r3578_parameter_verdict_matrix'),
    supabase.rpc('founder_r3578_monthly_calibration_trend'),
    supabase.rpc('founder_r3578_capa_status_board'),
    supabase.rpc('founder_r3578_root_cause_pareto'),
    supabase.rpc('founder_r3578_accuracy_impact_digest'),
    supabase.rpc('founder_r3578_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        UV-C Disinfection Robot (UVGI) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital UV-C / UVGI disinfection-robot QA log — device model (pulsed-xenon &amp; continuous
        mercury emitters) &times; parameter (UV dose mJ/cm&sup2;, lamp output &micro;W/cm&sup2;, cycle
        time, coverage %, motion-safety sensor, lamp hours) &times; reference vs measured &times;
        deviation % &times; tolerance &times; calibration date &amp; CAPA closure. Founder-gated view:
        QC verdicts, device-model scorecards, parameter &times; verdict matrix, root-cause pareto, and
