...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3455_qc_verdict_rollup'),
    supabase.rpc('founder_r3455_device_model_scorecard'),
    supabase.rpc('founder_r3455_parameter_verdict_matrix'),
    supabase.rpc('founder_r3455_monthly_accuracy_trend'),
    supabase.rpc('founder_r3455_capa_status_board'),
    supabase.rpc('founder_r3455_root_cause_pareto'),
    supabase.rpc('founder_r3455_accuracy_impact_digest'),
    supabase.rpc('founder_r3455_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Argon Plasma Coagulation (APC) Electrosurgery QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital argon plasma coagulation (APC) electrosurgery unit QC log — parameter (argon flow
        LPM, set &amp; delivered power W, ignition voltage kV, gas purity %, leakage current &micro;A)
        &times; device model &times; unit &times; reference vs measured &times; deviation % &times;
        tolerance band &times; calibration currency &amp; CAPA closure. Founder-gated view: QC
        verdicts, device-model scorecards, parameter &times; verdict matrix, accuracy trend,
