...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3558_qc_verdict_rollup'),
    supabase.rpc('founder_r3558_device_model_scorecard'),
    supabase.rpc('founder_r3558_parameter_verdict_matrix'),
    supabase.rpc('founder_r3558_monthly_qc_trend'),
    supabase.rpc('founder_r3558_capa_status_board'),
    supabase.rpc('founder_r3558_root_cause_pareto'),
    supabase.rpc('founder_r3558_accuracy_impact_digest'),
    supabase.rpc('founder_r3558_high_risk_queue'),
  ]);
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'tolerance_state', header: 'Tolerance' },
    { key: 'qc_verdict', header: 'Verdict' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Immunohematology Analyzer (Blood-Grouping) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital immunohematology analyzer QA log — blood grouping, cross-match &amp; Coombs on
        gel-card platforms. Tracks device model &times; parameter (reaction-grade accuracy,
        centrifuge RPM, incubation temp, optical read, carryover &amp; gel-card fill) &times; QC
        level (anti-A / anti-B / anti-D / control) &times; reference vs measured &times; deviation %
        &times; tolerance &times; calibration date &amp; CAPA closure. Founder-gated view: QC
        verdicts, device-model scorecards, root-cause pareto and accuracy-impact digest across NABL
        &amp; ISO 15189 surfaces.
      </p>
