...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3563_qc_verdict_rollup'),
    supabase.rpc('founder_r3563_device_model_scorecard'),
    supabase.rpc('founder_r3563_parameter_verdict_matrix'),
    supabase.rpc('founder_r3563_monthly_accuracy_trend'),
    supabase.rpc('founder_r3563_capa_status_board'),
    supabase.rpc('founder_r3563_root_cause_pareto'),
    supabase.rpc('founder_r3563_accuracy_impact_digest'),
    supabase.rpc('founder_r3563_high_risk_queue'),
  ]);
  ...
  const digestCols: Column<DigestRow>[] = [
    { key: 'severity_band', header: 'Severity Band' },
    { key: 'checks', header: 'Checks' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Cholangioscope (Biliary Endoscope) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital cholangioscope (biliary / SpyGlass endoscope) QC log — device model &times; unit
        &times; parameter (image resolution, light transmission, working-channel flow, viewing angle,
        color fidelity, seal-leak test) &times; reference vs measured value &times; accuracy deviation %
        &times; tolerance &amp; calibration currency &amp; CAPA closure. Founder-gated view: QC
        verdicts, device-model scorecards, parameter &times; verdict matrix, monthly accuracy trend,
