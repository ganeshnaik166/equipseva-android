...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3495_qc_verdict_rollup'),
    supabase.rpc('founder_r3495_device_model_scorecard'),
    supabase.rpc('founder_r3495_parameter_verdict_matrix'),
    supabase.rpc('founder_r3495_monthly_accuracy_trend'),
    supabase.rpc('founder_r3495_capa_status_board'),
    supabase.rpc('founder_r3495_root_cause_pareto'),
    supabase.rpc('founder_r3495_accuracy_impact_digest'),
    supabase.rpc('founder_r3495_high_risk_queue'),
  ]);
  ...
  const digestCols: Column<DigestRow>[] = [
    ...
    { key: 'out_of_tolerance', header: 'Out of Tol' },
    { key: 'avg_deviation_pct', header: 'Avg Dev %' },
    { key: 'max_abs_deviation_pct', header: 'Max Abs Dev %' },
    { key: 'failed', header: 'Failed' },
  ];
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Gel Electrophoresis / Gel-Documentation (Lab) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital lab gel electrophoresis &amp; gel-documentation imaging QC — power-supply set/measured
        voltage &amp; current, UV transilluminator intensity, band resolution &amp; CCD camera linearity
        &times; device model &times; reference vs measured value &times; deviation % &times; tolerance
        &times; verdict &amp; CAPA closure. Founder-gated view: QC verdicts, device-model scorecards,
        parameter &times; verdict matrix, monthly accuracy trend, root-cause pareto and accuracy-impact
        digest across NABH, CDSCO &amp; ISO 15189 surfaces.
      </p>
