...
export const metadata = { title: "AMC renewal window 30d — EquipSeva Founder Console" };
...
export default async function AmcRenewalWindowPage() {
  await requireFounder();
  const supabase = await getSupabaseServerClient();
  const { data, error } = await supabase.rpc("founder_amc_renewal_window_30d");
  if (error) throw new Error(`founder_amc_renewal_window_30d: ${error.message}`);
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-baseline justify-between">
        <h1 className="text-xl font-semibold">AMC renewal window 30d</h1>
        <span className="text-xs text-[var(--color-muted)]">
          Renewing in next 30d · grand MRR at risk: <span className="font-mono tabular-nums text-[var(--color-warn)]">{formatRupees(grandMRR)}</span>
        </span>
      </header>
      <DataTable columns={cols} rows={rows} rowKey={(r) => r.tier} emptyMessage="No AMC contracts renewing in next 30 days." />
    </div>
  );
}
