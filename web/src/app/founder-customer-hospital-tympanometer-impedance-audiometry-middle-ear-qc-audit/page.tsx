...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3454_qc_verdict_rollup'),
    supabase.rpc('founder_r3454_device_model_scorecard'),
    supabase.rpc('founder_r3454_parameter_verdict_matrix'),
    supabase.rpc('founder_r3454_monthly_accuracy_trend'),
    supabase.rpc('founder_r3454_capa_status_board'),
    supabase.rpc('founder_r3454_root_cause_pareto'),
    supabase.rpc('founder_r3454_accuracy_impact_digest'),
    supabase.rpc('founder_r3454_high_risk_queue'),
  ]);
  ...
  const digestCols: Column<DigestRow>[] = [
    { key: 'parameter', header: 'Parameter' },
    { key: 'checks', header: 'Checks' },
    { key: 'within_tol', header: 'Within Tolerance' },
    { key: 'out_of_tol', header: 'Out of Tolerance' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Tympanometer / Middle-Ear Impedance Audiometry QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital tympanometer / middle-ear impedance analyzer QC log — parameter (ear-canal pressure daPa,
        static compliance ml, ear-canal volume ml, probe-tone dB, acoustic reflex threshold dB &amp;
        tympanometric gradient daPa) &times; device model &times; reference vs measured &times; deviation %
        &times; tolerance band &times; calibration currency &amp; CAPA closure. Founder-gated view: QC
        verdicts, device-model scorecards, parameter &times; verdict matrix, root-cause pareto, and
        accuracy-impact digest across NABH &amp; CDSCO surfaces.
      </p>
