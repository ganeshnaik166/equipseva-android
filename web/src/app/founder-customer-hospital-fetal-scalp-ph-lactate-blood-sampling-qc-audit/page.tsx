...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3538_qc_verdict_rollup'),
    supabase.rpc('founder_r3538_device_model_scorecard'),
    supabase.rpc('founder_r3538_parameter_verdict_matrix'),
    supabase.rpc('founder_r3538_monthly_calibration_trend'),
    supabase.rpc('founder_r3538_capa_status_board'),
    supabase.rpc('founder_r3538_root_cause_pareto'),
    supabase.rpc('founder_r3538_accuracy_impact_digest'),
    supabase.rpc('founder_r3538_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Fetal-Scalp pH / Lactate Blood-Sampling QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Intrapartum fetal-scalp blood-sampling analyzer QA log — parameter (pH accuracy, lactate
        mmol accuracy, sample temperature, carryover %, repeatability CV &amp; sample volume)
        &times; reference vs. measured value &times; deviation % &times; within-tolerance &times;
        calibration date &times; QC verdict &amp; CAPA closure. Founder-gated view: QC verdicts,
        device-model scorecards, parameter &times; verdict matrix, root-cause pareto, and
        accuracy-impact digest across NABH &amp; CDSCO / ISO 15189 surfaces.
      </p>
