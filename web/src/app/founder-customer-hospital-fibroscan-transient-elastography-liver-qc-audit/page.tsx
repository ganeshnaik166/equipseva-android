...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3519_qc_verdict_rollup'),
    supabase.rpc('founder_r3519_device_model_scorecard'),
    supabase.rpc('founder_r3519_parameter_verdict_matrix'),
    supabase.rpc('founder_r3519_monthly_qc_trend'),
    supabase.rpc('founder_r3519_capa_status_board'),
    supabase.rpc('founder_r3519_root_cause_pareto'),
    supabase.rpc('founder_r3519_accuracy_impact_digest'),
    supabase.rpc('founder_r3519_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        FibroScan Transient-Elastography (Liver) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital FibroScan transient-elastography QA log — liver stiffness (kPa), CAP (dB/m), probe
        frequency (MHz), IQR/median ratio, success rate &amp; shear-wave speed &times; device model
        (FibroScan 630 Expert, 530 Compact, 430 Mini, Touch 502) &times; reference-vs-measured accuracy
        &times; deviation % &times; within-tolerance &times; calibration date &amp; CAPA closure.
