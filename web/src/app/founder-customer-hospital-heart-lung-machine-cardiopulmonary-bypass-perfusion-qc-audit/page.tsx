...
type RiskRow = {
  ...
  safety_interlock_ok: boolean | null;
  notes: string | null;
};
...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3442_qc_verdict_rollup'),
    supabase.rpc('founder_r3442_device_model_scorecard'),
    supabase.rpc('founder_r3442_parameter_verdict_matrix'),
    supabase.rpc('founder_r3442_monthly_accuracy_trend'),
    supabase.rpc('founder_r3442_capa_status_board'),
    supabase.rpc('founder_r3442_root_cause_pareto'),
    supabase.rpc('founder_r3442_accuracy_impact_digest'),
    supabase.rpc('founder_r3442_high_risk_queue'),
  ]);
  ...
  const riskCols: Column<RiskRow>[] = [
    ...
    { key: 'tolerance_pct', header: 'Tolerance %' },
    { key: 'safety_interlock_ok', header: 'Interlock OK' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Heart-Lung Machine / Cardiopulmonary-Bypass (CPB) Perfusion QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Cardiopulmonary-bypass perfusion QA log — device model (LivaNova, Terumo, Maquet,
        Medtronic Bio-Console) &times; parameter (pump flow L/min, gas-blender FiO2, heater-cooler
        temp, pressure alarm, level/bubble sensor, ACT timer) &times; reference vs measured &times;
        deviation % &times; tolerance band &times; safety interlock &times; calibration currency
        &amp; CAPA closure. Founder-gated view: QC verdicts, device-model scorecards, root-cause
        pareto, and accuracy-impact digest across NABH &amp; CDSCO surfaces.
      </p>
