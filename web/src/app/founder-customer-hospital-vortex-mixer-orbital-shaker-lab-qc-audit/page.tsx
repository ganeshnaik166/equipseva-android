...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3499_qc_verdict_rollup'),
    supabase.rpc('founder_r3499_device_model_scorecard'),
    supabase.rpc('founder_r3499_parameter_verdict_matrix'),
    supabase.rpc('founder_r3499_monthly_accuracy_trend'),
    supabase.rpc('founder_r3499_capa_status_board'),
    supabase.rpc('founder_r3499_root_cause_pareto'),
    supabase.rpc('founder_r3499_accuracy_impact_digest'),
    supabase.rpc('founder_r3499_high_risk_queue'),
  ]);
  ...
  const digestCols: Column<DigestRow>[] = [
    ...
    { key: 'out_of_tolerance', header: 'Out of Tolerance' },
    { key: 'avg_deviation_pct', header: 'Avg |Dev| %' },
    { key: 'max_deviation_pct', header: 'Worst |Dev| %' },
  ];
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Vortex Mixer / Orbital Shaker (Lab) QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital laboratory vortex mixer &amp; orbital shaker QC log — parameter (set &amp; measured
        RPM, orbit diameter mm, timer accuracy, platform temperature, speed stability) &times;
        reference vs measured value &times; deviation % &times; tolerance &times; calibration date
        &times; lab section &times; device model &amp; CAPA closure. Founder-gated view: QC verdicts,
        device-model scorecards, parameter &times; verdict matrix, monthly accuracy trend,
        root-cause pareto, and accuracy-impact digest across NABL &amp; ISO 15189 surfaces.
      </p>
