...
export const metadata = { title: "Spare parts stuck — aging — EquipSeva Founder Console" };
...
export default async function SparePartsStuckAgingPage() {
  await requireFounder();
  const supabase = await getSupabaseServerClient();
  const { data, error } = await supabase.rpc("founder_spare_parts_stuck_aging");
  if (error) throw new Error(`founder_spare_parts_stuck_aging: ${error.message}`);
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-baseline justify-between">
        <h1 className="text-xl font-semibold">Spare parts stuck — aging</h1>
        <span className="text-xs text-[var(--color-muted)]">
          Paid orders not shipped/delivered: <span className="font-mono tabular-nums">{formatNumber(totalCnt)}</span> · {formatRupees(totalInr)} · <span className="text-[var(--color-danger)]">{formatNumber(stuckLong)} stuck &gt;7d</span>
        </span>
      </header>
      <DataTable columns={cols} rows={rows} rowKey={(r) => r.bucket} emptyMessage="No paid orders awaiting shipment." />
    </div>
  );
}
