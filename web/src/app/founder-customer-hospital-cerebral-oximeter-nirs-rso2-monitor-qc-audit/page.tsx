...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3527_qc_verdict_rollup'),
    supabase.rpc('founder_r3527_device_model_scorecard'),
    supabase.rpc('founder_r3527_parameter_verdict_matrix'),
    supabase.rpc('founder_r3527_monthly_calibration_trend'),
    supabase.rpc('founder_r3527_capa_status_board'),
    supabase.rpc('founder_r3527_root_cause_pareto'),
    supabase.rpc('founder_r3527_accuracy_impact_digest'),
    supabase.rpc('founder_r3527_high_risk_queue'),
  ]);
  ...
  const digestCols: Column<DigestRow>[] = [
    { key: 'parameter', header: 'Parameter' },
    { key: 'checks', header: 'Checks' },
    { key: 'avg_reference_value', header: 'Avg Reference' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Cerebral Oximeter (NIRS / rSO2) Monitor QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Cerebral oximeter (near-infrared spectroscopy, regional rSO2) monitor QA log — device model
        &times; unit &times; QC parameter (rSO2 accuracy, LED intensity, signal-quality index, baseline
        drift, response time &amp; sensor impedance) &times; reference vs measured &times; deviation %
        &times; tolerance &times; probe condition &times; ambient-light &amp; calibration currency with
        CAPA closure. Founder-gated view: QC verdicts, device-model scorecards, root-cause pareto, and
