...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3551_qc_verdict_rollup'),
    supabase.rpc('founder_r3551_device_model_scorecard'),
    supabase.rpc('founder_r3551_parameter_verdict_matrix'),
    supabase.rpc('founder_r3551_monthly_accuracy_trend'),
    supabase.rpc('founder_r3551_capa_status_board'),
    supabase.rpc('founder_r3551_root_cause_pareto'),
    supabase.rpc('founder_r3551_accuracy_impact_digest'),
    supabase.rpc('founder_r3551_high_risk_queue'),
  ]);
  ...
  const digestCols: Column<DigestRow>[] = [
    { key: 'parameter', header: 'Parameter' },
    { key: 'checks', header: 'Checks' },
    { key: 'within_tolerance_count', header: 'Within Tolerance' },
    { key: 'out_of_tolerance_count', header: 'Out of Tolerance' },
    { key: 'avg_deviation_pct', header: 'Avg Deviation %' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Thoracoscope (VATS) Endoscope QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital thoracoscope (VATS) endoscope imaging QC log — parameter (image resolution, light
        transmission, viewing angle, color fidelity, focus clarity &amp; seal leak test) &times;
        reference vs measured value &times; deviation % &times; within-tolerance &times; calibration
        date &times; QC verdict &amp; CAPA closure. Founder-gated view: QC verdicts, device-model
        scorecards, parameter matrix, accuracy trend, root-cause pareto, and an out-of-tolerance /
        leak-fail high-risk queue across NABH &amp; CDSCO surfaces.
      </p>
