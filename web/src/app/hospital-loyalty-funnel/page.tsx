...
export const metadata = { title: "Hospital loyalty funnel — EquipSeva Founder Console" };
export const dynamic = "force-dynamic";

type Row = { stage: string; cnt: number; pct_signup: number };

export default async function HospitalLoyaltyFunnelPage() {
  await requireFounder();
  const supabase = await getSupabaseServerClient();
  const { data, error } = await supabase.rpc("founder_hospital_loyalty_funnel");
  if (error) throw new Error(`founder_hospital_loyalty_funnel: ${error.message}`);
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-baseline justify-between">
        <h1 className="text-xl font-semibold">Hospital loyalty funnel</h1>
        <span className="text-xs text-[var(--color-muted)]">Lifetime cohort · signup → 10+ jobs loyalist</span>
      </header>
      <DataTable columns={cols} rows={rows} rowKey={(r) => r.stage} emptyMessage="No hospitals." />
    </div>
  );
}
