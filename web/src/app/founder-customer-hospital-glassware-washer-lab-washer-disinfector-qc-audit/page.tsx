...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3490_qc_verdict_rollup'),
    supabase.rpc('founder_r3490_device_model_scorecard'),
    supabase.rpc('founder_r3490_parameter_verdict_matrix'),
    supabase.rpc('founder_r3490_monthly_calibration_trend'),
    supabase.rpc('founder_r3490_capa_status_board'),
    supabase.rpc('founder_r3490_root_cause_pareto'),
    supabase.rpc('founder_r3490_accuracy_impact_digest'),
    supabase.rpc('founder_r3490_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Laboratory Glassware Washer-Disinfector QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital laboratory glassware washer-disinfector QA log — parameter (wash temperature,
        rinse conductivity, A0 disinfection value, drying temperature, cycle time, detergent dose)
        &times; device model &times; reference vs measured value &times; deviation % &times;
        tolerance &amp; verdict, with CAPA closure. Founder-gated view: QC verdicts, device-model
        scorecards, parameter &times; verdict matrix, root-cause pareto, and accuracy-impact digest
        across NABL &amp; NABH surfaces.
      </p>
