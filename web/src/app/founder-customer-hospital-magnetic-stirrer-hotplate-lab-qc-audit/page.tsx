...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3494_qc_verdict_rollup'),
    supabase.rpc('founder_r3494_device_model_scorecard'),
    supabase.rpc('founder_r3494_parameter_verdict_matrix'),
    supabase.rpc('founder_r3494_monthly_calibration_trend'),
    supabase.rpc('founder_r3494_capa_status_board'),
    supabase.rpc('founder_r3494_root_cause_pareto'),
    supabase.rpc('founder_r3494_accuracy_impact_digest'),
    supabase.rpc('founder_r3494_high_risk_queue'),
  ]);
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Laboratory Magnetic Stirrer / Hotplate QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital laboratory magnetic stirrer &amp; hotplate QA log — device model &times; lab section
        &times; parameter (set/measured RPM, set/measured temperature, temperature uniformity, timer
        accuracy) &times; reference vs measured value &times; deviation % &times; tolerance limit
        &times; calibration currency &amp; CAPA closure. Founder-gated view: QC verdicts, device-model
        scorecards, parameter &times; verdict matrix, accuracy-impact digest, root-cause pareto, and
        regulatory surfaces across NABL &amp; ISO 15189 lab compliance.
      </p>
