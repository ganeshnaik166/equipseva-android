...
export default async function Page() {
  ...
  const [
    ...
    supabase.rpc('founder_r3526_qc_verdict_rollup'),
    supabase.rpc('founder_r3526_device_model_scorecard'),
    supabase.rpc('founder_r3526_parameter_verdict_matrix'),
    supabase.rpc('founder_r3526_monthly_calibration_trend'),
    supabase.rpc('founder_r3526_capa_status_board'),
    supabase.rpc('founder_r3526_root_cause_pareto'),
    supabase.rpc('founder_r3526_accuracy_impact_digest'),
    supabase.rpc('founder_r3526_high_risk_queue'),
  ]);
  ...
  const impactCols: Column<ImpactRow>[] = [
    { key: 'regulatory_impact', header: 'Regulatory / Accuracy Impact' },
    { key: 'findings', header: 'Findings' },
  ...
  return (
    <main style={{ padding: '2rem', maxWidth: 1400, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>
        Transcutaneous PO2 / PCO2 Monitor QC Audit
      </h1>
      <p style={{ color: '#555', marginBottom: '2rem' }}>
        Hospital transcutaneous PO2/PCO2 (tcpO2/tcpCO2) monitor QA log — device model &times;
        parameter (tcpO2 mmHg, tcpCO2 mmHg, sensor temperature, drift per hour, membrane response,
        calibration-gas accuracy) &times; reference vs measured value &times; deviation % &times;
        tolerance &times; calibration date &times; QC verdict &amp; CAPA closure. Founder-gated
