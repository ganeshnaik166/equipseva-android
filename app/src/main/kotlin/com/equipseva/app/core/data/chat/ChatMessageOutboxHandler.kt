 ...
 *
 * **Owner gate:** the payload carries the original `senderUserId`. Outbox
 * rows outlive sign-out/sign-in, so on a shared device user B could otherwise
 * drain user A's queued writes. Before dispatch we compare the payload's
 * `senderUserId` against the current `auth.uid` and:
 * - `GiveUp` on mismatch (wrong user — don't impersonate, don't retry).
 * - `Retry` on no-session (signed out — try again next flush).
 */
