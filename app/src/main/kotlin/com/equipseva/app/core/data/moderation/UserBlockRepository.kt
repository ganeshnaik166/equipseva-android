...
@Singleton
class UserBlockRepository @Inject constructor(
    ...
    suspend fun unblock(blockedUserId: String): Result<Unit> = runCatching {
        val userId = requireNotNull(client.auth.currentUserOrNull()?.id) { "Not signed in" }
    ...
    suspend fun isBlocked(blockedUserId: String): Result<Boolean> = runCatching {
