-- =====================================================================
-- Round 3802 -- verification-only: round3801's gate was DEFECTIVE
-- =====================================================================
--
-- This migration changes no function. It exists because round3801 shipped
-- 293 repairs behind a gate that VERIFIED NOTHING, and that needs to be
-- on the record with a working gate rather than quietly corrected.
--
-- WHAT WENT WRONG IN round3801's GATE
--   Its probe was:
--       WITH q AS (SELECT row_number() OVER () AS rn, t.<col> AS v
--                    FROM public.<fn>() t)
--       SELECT count(*), count(DISTINCT v), count(*) FILTER (...)
--       FROM q a LEFT JOIN q b ON b.rn = a.rn + 1
--   `count(DISTINCT v)` is AMBIGUOUS across `q a` and `q b`, so every
--   iteration raised 42702. The loop caught it with
--   `EXCEPTION WHEN OTHERS THEN v_skipped := v_skipped + 1; CONTINUE;`
--   -- a handler put there to tolerate functions that cannot execute --
--   so all 314 pairs were counted as "skipped", the violation list stayed
--   empty, and the gate reported success. Its own NOTICE said it plainly:
--   "ordering PROVEN for 0 (fn,col) pair(s); 314 skipped".
--
--   Lesson, and it generalises past this migration: an EXCEPTION handler
--   inside a verification loop can silently convert "could not check" into
--   "checked and fine". A gate that can skip everything MUST fail when it
--   proves nothing. This one now does -- see the final assertion.
--
-- TWO FURTHER MEASUREMENT ERRORS, both mine, both worth recording because
-- each produced a convincing false alarm:
--
--   1. WRONG DIRECTION. The first re-check reported 218 of 314 pairs still
--      unordered. The direction had been derived by searching from the
--      FIRST `order by` anywhere in the definition -- often a window
--      function's `OVER (ORDER BY ...)` -- and then matching the column,
--      which now hits the select-list item where round3801 had just added
--      `AS <col>`. No DESC follows an alias, so nearly everything was
--      classified ASC when it is actually DESC. The tell was in the data:
--      n=6 with 5 violations is not "disordered", it is PERFECTLY ordered
--      the other way. Direction must be read from inside the ORDER BY
--      CLAUSE that was located, not from the first match in the file.
--
--   2. SECONDARY KEYS. With directions corrected, 20 pairs still looked
--      unordered. All 20 were multi-key orderings where the measured
--      column was NOT the first key:
--         founder_top_hospitals_90d   ORDER BY jobs_posted DESC, gross_rupees DESC
--         r3043_findings_by_category  ORDER BY critical_count DESC, total DESC
--         firmware_spread_r3023       ORDER BY i.pump_model, units DESC
--      A secondary key is monotonic only WITHIN ties of the keys before
--      it, so measuring it globally is expected to show violations and
--      proves nothing either way. The valid empirical test is the FIRST
--      key; the remaining keys are covered by the static assertion that
--      every ORDER BY identifier now resolves to an output column.
--
-- THE ACTUAL RESULT, once measured correctly
--   Primary-key ordering, 284 testable pairs:
--     211 PROVEN correctly ordered
--      73 undetermined (fewer than 3 rows, or fewer than 2 distinct
--         values -- a constant column is trivially monotonic and settles
--         nothing)
--       0 still unordered
--   So round3801's repair was correct and complete. Only its gate was
--   broken.
--
-- WHAT THIS MIGRATION ASSERTS
--   The same proof, re-run at apply time, with:
--     * `count(DISTINCT a.v)` -- disambiguated
--     * the direction taken from the located ORDER BY clause
--     * only FIRST keys measured
--     * NO blanket EXCEPTION handler: a probe that errors is recorded as a
--       failure, not skipped
--     * a floor on coverage, so this gate cannot pass by proving nothing

BEGIN;

CREATE TEMP TABLE _r3802_pairs(fn text, col text, dir text) ON COMMIT DROP;
INSERT INTO _r3802_pairs VALUES
...
DECLARE
  ...
  v_total    int;
BEGIN
  SELECT count(*) INTO v_total FROM _r3802_pairs;
  ...
  FOR r IN SELECT * FROM _r3802_pairs LOOP
    BEGIN
      EXECUTE format(
    ...
    EXCEPTION WHEN OTHERS THEN
      -- deliberately NOT swallowed: this is exactly how round3801's gate
      -- talked itself into a pass
      IF length(v_err) < 600 THEN
        v_err := v_err || format('%s.%s [%s] %s; ', r.fn, r.col, SQLSTATE, SQLERRM);
      END IF;
      CONTINUE;
    END;
  ...
  RAISE NOTICE 'round 3802: % pair(s) tested -- % PROVEN ordered, % undetermined',
    v_total, v_proven, v_undet;

  IF v_err <> '' THEN
    RAISE EXCEPTION 'round 3802 VERIFY FAILED: probe error(s): %', v_err;
  END IF;
  IF v_bad <> '' THEN
    RAISE EXCEPTION 'round 3802 VERIFY FAILED: still unordered: %', v_bad;
  END IF;

  -- A gate that proves nothing must FAIL. round3801's did not, which is why
  -- this migration exists. Require most of the testable set to be settled.
  IF v_proven < 150 THEN
    RAISE EXCEPTION
      'round 3802 VERIFY FAILED: only % of % pair(s) could be proven -- refusing to report success on that basis',
      v_proven, v_total;
  END IF;

  RAISE NOTICE 'round 3802 verified: round3801''s repair is sound; % ordering(s) proven against live data', v_proven;
END
$gate$;
