BEGIN;
DROP FUNCTION IF EXISTS public.founder_amc_revenue_by_month_by_tier();
CREATE OR REPLACE FUNCTION public.founder_amc_revenue_by_month_by_tier()
RETURNS TABLE (
  month_ist   date,
  tier        text,
  orders      bigint,
...
BEGIN
  ...
  SELECT
    ...
    coalesce((SELECT count(*)::bigint FROM public.amc_payment_orders o
             ...
             WHERE o.status='paid' AND c.amc_tier = t.tier
...
END;
$$;
REVOKE EXECUTE ON FUNCTION public.founder_amc_revenue_by_month_by_tier() FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.founder_amc_revenue_by_month_by_tier() TO authenticated;
COMMIT;
