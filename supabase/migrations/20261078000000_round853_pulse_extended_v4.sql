-- Round 853 — /pulse-extended v4 adds 2 integrity KPIs (20 total).
--
-- r840 shipped 18 (v3). Adding 2 more high-signal metrics that the
-- founder cares about during daily review now that the anti-mod stack
-- is live (r843-r851):
--   19. Integrity checks (verify-play-integrity calls; spike = surge
--       in app installs OR a script automating fake reqs)
--   20. Dirty header reports (X-Equipseva-Integrity self-reports
--       carrying tampered / root=1 / frida=1 — early signal before
--       Google verdict catches up)
BEGIN;
...
BEGIN
  ...
  WITH rows(metric, ord, t, l) AS (
    ...
    ('Integrity checks', 19,
      (SELECT count(*)::numeric FROM public.device_integrity_checks WHERE created_at >= v_cur_start AND created_at < now()),
      (SELECT count(*)::numeric FROM public.device_integrity_checks WHERE created_at >= v_prior_start AND created_at < v_cur_start)),
    ('Dirty header reports', 20,
      (SELECT count(*)::numeric FROM public.device_integrity_checks
        WHERE created_at >= v_cur_start AND created_at < now()
          AND (client_integrity_header LIKE '%tampered%'
            OR client_integrity_header LIKE '%root=1%'
            OR client_integrity_header LIKE '%frida=1%')),
      (SELECT count(*)::numeric FROM public.device_integrity_checks
