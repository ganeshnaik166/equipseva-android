-- engineer_public_profile is SECURITY DEFINER and returns phone + email
-- to any authenticated caller. By design on the engineer-contact-channels
-- branch — hospitals must be able to reach engineers — but the surface
-- is open to enumeration scraping. Any signed-in account can iterate
-- engineer_id values and harvest the verified-engineer phone/email list.
--
-- Tighten: contact fields (phone, email) are returned only when the
-- caller has an established relationship with the engineer:
--   - the caller is the engineer themselves, OR
--   - the caller is the hospital_user_id on any repair_jobs row whose
--     engineer_id resolves to this engineer (a real or completed booking
--     exists), OR
--   - the caller is a participant of an existing chat_conversation
--     with the engineer's user_id (a chat link was already opened), OR
--   - the caller is an admin / founder.
-- Otherwise the contact columns are nulled out and the rest of the
-- public profile (name, city, specializations, ratings, bio, hourly
-- rate) still renders so directory browsing keeps working.
