-- Round 727 — fix /commission-revenue-30d platform-take coefficient
--
-- r663 used 0.15 (15%) as the platform take, but that's the AMC-visit
-- split (per r464 + r626). For non-AMC repair jobs the platform take
-- is 0.07 (7%) per r305 / r626 commission_rate(). Live data confirmed:
-- a ₹20 repair job paid out ₹18.60 to engineer = 7% take.
--
-- For now use 0.07 since most jobs are non-AMC. AMC-visit volume blends
-- to a higher effective rate; the page label keeps the (est) suffix.
BEGIN;
...
BEGIN
  ...
  SELECT
    ...
    coalesce(
      (SELECT round(sum(rj.contracted_amount_rupees) * 0.07, 2)::numeric FROM public.repair_jobs rj
       WHERE rj.status = 'completed'
         AND (rj.completed_at AT TIME ZONE 'Asia/Kolkata')::date = d.day_ist
