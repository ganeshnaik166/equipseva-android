-- Round 734 — fix decide_cost_revision invalid enum literal (breaks ALL
-- cost-revision approve/reject).
--
-- Found by the r1401-era contract-mismatch audit (adversarially verified):
-- decide_cost_revision (latest def = round 465, 20260721100000:220-223)
-- fetches the row with `WHERE id = p_revision_id AND status = 'pending'`.
-- But repair_cost_revision_status (20260504120000:22-27) has ONLY the labels
-- proposed / approved / rejected / expired — there is no 'pending'. Comparing
-- the enum column to the unknown literal 'pending' forces a coercion via
-- enum_in() and raises 22P02 "invalid input value for enum
-- repair_cost_revision_status: \"pending\"" on EVERY call, before any row is
-- examined. So every hospital Approve/Reject of an engineer's revised quote
-- (CostRevisionRepository.decide -> rpc decide_cost_revision) fails 100% of
-- the time. Same class as the r733 length(equipment_category) enum outage.
--
-- Round 465 introduced the regression; the prior idempotent version
-- (20260626150000) selected on id alone. The correct "awaiting decision" label
-- is 'proposed' (the table default, and the partial unique index
-- one_pending_revision_per_job keys on status = 'proposed').
--
-- Fix: CREATE OR REPLACE with the single literal corrected to 'proposed';
-- function body otherwise identical to round 465. No client change needed.
-- Transactional (fails safe — a bad run rolls back, live function intact).
BEGIN;
...
BEGIN
  ...
  -- r734: was `status = 'pending'` (not a valid enum label -> 22P02 on every
  -- call). 'proposed' is the awaiting-decision state.
