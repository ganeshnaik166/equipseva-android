BEGIN;
-- r1303 — founder_action_center: unified action queue across all founder domains.
-- Returns top items sorted by priority_rank — what the founder must do right now.
-- Each row has: source domain, item kind, item id (for click-through), severity,
-- age in hours, money_at_stake_inr (when relevant), human-readable label.
--
-- 14 source domains contribute:
--   • engineer_payouts (stuck >7d)
--   • code_red_requests (unresolved >4h)
--   • dispute_evidence_packs (open >7d)
--   • repair_job_escrow (held >14d / in_dispute)
--   • spare_part_orders (paid-not-shipped >7d)
--   • amc_contracts (paused >30d / renewing 30d)
--   • engineers (KYC pending >7d)
--   • profiles role=hospital (never posted job, >30d signup age)
--   • refund_authorization_requests (open >3d)
--   • collusion_flags (open critical-severity)
--   • dpdp_grievances (open + over 30-day SLA)
--   • amc_sla_breaches (today)
--   • risk_score_snapshots (critical band, action_taken=alert_only)
--   • spot_audit_invitations (pending response >7d)
--
-- Severity ranks (lower = higher priority):
--   1=critical · 2=high · 3=medium · 4=low

DROP FUNCTION IF EXISTS public.founder_action_center();
...
BEGIN
  ...
  WITH actions AS (
    ...
    -- 9. Collusion flags critical
    SELECT
      'collusion',
      'collusion_flag',
      f.id,
      ('Collusion flag · ' || f.signal_kind || ' · ' || coalesce(f.severity, 'unknown'))::text,
      1,
    ...
    WHERE f.status IN ('open', 'investigating')
      AND coalesce(f.severity, '') IN ('critical', 'high')

    UNION ALL

    -- 10. DPDP grievances over 30-day SLA
    SELECT
      'dpdp',
    ...
    WHERE coalesce(g.status, 'open') IN ('open', 'in_progress')
      AND g.created_at < now() - interval '30 days'

    UNION ALL

    -- 11. AMC SLA breaches today (IST)
    SELECT
      'amc_sla',
      ...
      2,
      extract(hour from (now() - b.created_at))::int,
      coalesce(b.credit_amount_rupees, 0)::numeric,
      b.created_at
    FROM public.amc_sla_breaches b
    WHERE b.created_at >= (now() AT TIME ZONE 'Asia/Kolkata')::date::timestamptz AT TIME ZONE 'Asia/Kolkata'
