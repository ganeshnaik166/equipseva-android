BEGIN;
DROP FUNCTION IF EXISTS public.founder_disputes_cumulative();
CREATE OR REPLACE FUNCTION public.founder_disputes_cumulative()
RETURNS TABLE (
  month_ist     date,
  submitted     bigint,
  cum_submitted bigint,
  accepted      bigint,
  rejected      bigint
)
...
END;
$$;
REVOKE EXECUTE ON FUNCTION public.founder_disputes_cumulative() FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.founder_disputes_cumulative() TO authenticated;
COMMIT;
