    ...
    SELECT
      ...
      coalesce((SELECT count(*)::bigint FROM public.engineer_payouts p
                WHERE p.status IN ('processed','paid')
                  AND date_trunc('month', (p.queued_at AT TIME ZONE 'Asia/Kolkata'))::date = m.month_ist), 0) AS paid_count,
      coalesce((SELECT sum(amount_inr)::numeric FROM public.engineer_payouts p
                WHERE p.status IN ('processed','paid')
                  AND date_trunc('month', (p.queued_at AT TIME ZONE 'Asia/Kolkata'))::date = m.month_ist), 0) AS paid_rupees
    FROM months m
  )
  SELECT
    m.month_ist,
